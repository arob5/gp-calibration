...
\end{align*}
Once again, we utilize the GP approximation so that $\tau$ is updated by sampling from the approximate conditional 
\[\tau \sim G\left(a + n/2, b + \hat{T}(x, z, u)/2\right) \]
where $\hat{T}(x, z, u)$ is a value sampled from the predictive GP distribution at the current value of $u$. 
...
\subsubsection{Complete Algorithm}
 ...
 \begin{algorithm}[H]
	...
	Calculate approximate likelihoods: \\
	 $\hat{\mathcal{L}}^{(t - 1)} := \left(\tau^{(t - 1)}\right)^{n/2} \exp\left\{-\frac{\tau^{(t - 1)}}{2} T^{(t - 1)}  \right\}$ \\ 
	 $\hat{\mathcal{L}}^{(t)} := \left(\tau^{(t - 1)}\right)^{n/2} \exp\left\{-\frac{\tau^{(t - 1)}}{2} T^{(t)} \right\}$  \\
	Accept $u^{(t)}$ with probability $\min\left\{1, \frac{\hat{\mathcal{L}}^{(t)}\pi_0(u^{(t)}) N(u^{(t - 1)}|u^{(t)}, \Sigma)}{\hat{\mathcal{L}}^{(t - 1)} \pi_0(u^{(t - 1)}) N(u^{(t)}|u^{(t - 1)}, \Sigma)} \right\}$ \\
	If accepted, also update sampled sufficient statistic $T^{(t)} := T^{(t - 1)}$
...
\subsubsection{Questions}
\begin{itemize}
\item The above algorithm is actually slightly different from the code. In the code, to define $\hat{\mathcal{L}}^{(t)}$ a new value of $\tau$ is first sampled from $G\left(a + n/2, b + T^{(t)}/2\right)$
and then this sampled value is used in calculating $\hat{\mathcal{L}}^{(t)}$, instead of $\tau^{(t - 1)}$ as I have in the above algorithm. I'm wondering why this is done instead of just conditioning 
on the current value $\tau^{(t - 1)}$. 
\end{itemize}
