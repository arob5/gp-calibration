...
\item The fact that I am differentiating between parameters $x$ and $u$ means that each (exact) likelihood evaluation (likelihood for the entire observed 
data vector $z \in \R^n$) requires $n$ runs of the simulation, one run for each $x_i$. This may not accurately capture how the SIPNET or other models work, 
as I haven't looked into the specifics yet. 
\end{itemize}
