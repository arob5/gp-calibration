...
Among the various emulator calibration approaches, I believe you are using the ``modular'' approach described in Gramacy 8.1.2 and originally in 
...
For now I am considering emulating the model outputs directly; I know that you emulate the sufficient statistics - I'll add this in once i have the basics down. The calibration approach is as follows. 
\begin{enumerate}
...
$x_i^m$ and $u_i^m$, respectively. Also let $y^m = (y_1^{u_1}, \dots, y_N^{u_N})$ be the vector of simulator outputs evaluated at the $N$ design points. 
...
in addition to the other parameters. Gramacy notes that the modular approach can help to avoid some of the pathologies sometimes present in the fully Bayesian approach. 

\textbf{TODO:} I don't think they're using this approach since they are emulating the discrepancy with field data and thus using the field data to fit the GP. 


\subsubsection{Questions}
\begin{itemize}
\item Am I correct in describing the calibration methodology used in PEcAn? 
