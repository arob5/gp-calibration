...
\end{align}
The dimension of this parameter vector can be reduced by accounting for the over-parameterization (e.g., by fixing 
$\fracRespiration$ and $\LAR$). The dynamics $\funcODE_{\Par}$ are nonlinear in $\state$ and autonomous, in that the 
...
= \LAR \cdot \stateV(\indexTime{\Time})
\end{align}
and
\begin{align}
\fwd_{\indexTime{\Time}}(\Par) 
&\Def \NPP(\stateV(\indexTime{\Time}), \forcing(\indexTime{\Time})) \\
