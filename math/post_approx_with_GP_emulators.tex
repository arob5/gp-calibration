...
\begin{align}
\fwdEmMeanDens[\Ndesign](\Par) &:= \priorDens(\Par) \Exp{\llik(\Par; \emMean[\Ndesign]{\fwd}(\Par))} \label{post_dens_mean_fwd} \\
\llikEmMeanDens[\Ndesign](\Par) &:= \priorDens(\Par) \Exp{\emMean[\Ndesign]{\llik}(\Par)} \label{post_dens_mean_llik}.
\end{align}
These approximations fail to account for the uncertainty introduced by the surrogate model. One approach to propagate the 
...
\end{align}
The subscripts in the expectations indicate integration with respect to the GP predictive distributions 
...
The term \textit{marginal approximation} follows the references \cite{StuartTeck1, StuartTeck2}, which analyze the error of these
approximations in the Gaussian likelihood setting. 
