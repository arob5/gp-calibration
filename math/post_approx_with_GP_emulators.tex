...
An intuitive design strategy is to simply select inputs where the quantity of 
interest exhibits high uncertainty. 
\begin{definition}
Given a map $\phi: \R \to [0,\infty)$, consider the random variable
 $\phi_{\Ndesign}(\Par) \Def \phi(\funcEm(\Par))$.
We define the \textbf{pointwise uncertainty criterion} with respect to $\phi$ to be the acquisition function
\begin{equation}
\acq[\Ndesign](\Par) \Def -\E\left[\phi_{\Ndesign}(\Par) \right].
\end{equation}
\end{definition}
The negation is included to align with our convention of minimizing acquisition functions.
Such a pointwise construction provides a method to define acquisition functions in the 
$\Nbatch = 1$ setting. For example, setting $\phi(\varphi) \Def [\varphi - \gpMean[\Ndesign](\Par)]^2$
gives the \textit{maximum variance} ($\maxvarLabel$) criterion
\begin{equation}
\labelAcq[\Ndesign]{\maxvarLabel}(\Par) \Def -\Var[\funcEm[\Ndesign](\Par)]  =  -\gpKer(\Par).
\end{equation}
A generalization of this idea to the batch setting requires the definition of a map 
$\phi: \R^{\Nbatch} \to [0, \infty)$ that encodes the uncertainty in an $\R^{\Nbatch}$-valued
random vector.
\begin{definition}
Given a map $\phi: \R^{\Nbatch} \to [0,\infty)$, consider the random variable
 $\phi_{\Ndesign}(\designBatchIn) \Def \phi(\funcEm(\designBatchIn))$.
We define the \textbf{local uncertainty criterion} with respect to $\phi$ to be the acquisition function
\begin{equation}
\acq[\Ndesign](\designBatchIn) \Def -\E\left[\phi_{\Ndesign}(\designBatchIn) \right].
\end{equation}
\end{definition}
A common example in the batch setting follows by choosing 
$\phi(\funcVal[\Nbatch]) \Def -\log \Gaussian(\funcVal[\Nbatch] \given \gpMean[\Ndesign](\designBatchIn), \gpKer[\Ndesign](\designBatchIn))$,
which yields the \textit{maximum entropy} ($\maxentLabel$) acquisition
\begin{equation}
\labelAcq[\Ndesign]{\maxentLabel}(\designBatchIn) \Def -\Ent[\funcEm[\Ndesign](\designBatchIn)]  =  -\frac{1}{2} \log\det(2\pi e \gpKer(\designBatchIn)),
\end{equation}
where $\Ent[Z]$ denotes the entropy of a random vector $Z$. These examples consider uncertainty in the GP 
...
$\funcVal[\Nbatch] \Def \funcEm(\designBatchIn)$ at a candidate set $\designBatchIn$ have not yet been observed. 
We therefore consider marginalizing this unknown quantity with respect to the current GP predictive distribution. 
This leads to the consideration of acquisition functions of the following form.
\begin{definition}
Let $\phi_N(\Par)$ be defined as above with respect to a map $\phi: \R \to [0,\infty)$. Moreover, let 
$\weightDens_{\Ndesign}: \Par \to [0,\infty)$ be a density on $\parSpace$. Then the \textbf{integrated conditional uncertainty}
criterion with respect to $\phi$ and $\weightDens_{\Ndesign}$ is given by
\begin{equation}
\acq[\Ndesign](\designIn[\Nbatch])
\Def \int_{\parSpace} \E_{\funcVal[\Nbatch]} \left[\phi_N(\Par) \given 
\funcEm[\Ndesign](\designBatchIn) = \designBatchFunc \right] \weightDens_{N}(\Par) d\Par.
\end{equation}
\end{definition}
In addition to considering the global effect of acquiring new points, notice that these criteria provide the means to 
extend a pointwise uncertainty criterion to the batch setting.
