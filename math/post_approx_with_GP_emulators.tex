...
unnormalized random density $\llikEmRdm[\Ndesign]{\postDens}$. Arguably the simplest 
approach is to reduce the stochastic surrogate to a deterministic one by considering only the 
GP predictive mean. We term the resulting posterior
\begin{align}
...
This approximation fail to account for the uncertainty introduced by the surrogate model, which can lead 
