...
\end{equation}
This objective strikes a balance between 
$\emMean[\Ndesign]{\llik}(\Par)$, $\emKer[\Ndesign]{\llik}(\Par)$, and $\priorDens(\Par)$, rather than only considering 
$\emKer[\Ndesign]{\llik}(\Par)$. However, it still suffers from the drawbacks associated with local uncertainty criteria, 
...
the current posterior approximation. Alternatively, we might consider weighting by the 
expected approximate posterior resulting from conditioning on 
$\{\designBatchIn, \designBatchLlik\}$. 
