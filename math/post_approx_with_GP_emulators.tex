...
\end{align}
Such an approximation is briefly noted in \citet{StuartTeck1} and \citet{VehtariParallelGP}.
We note that \cref{samp_dens_norm} is only well-defined if almost every realization from the likelihood 
surrogate induces a valid probability density, a property that will depend on the GP emulator.
...
literature opt for alternative posterior approximations. Given that the primary difficulties stem from the normalizing constant
