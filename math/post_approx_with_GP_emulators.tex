...
In the forward model emulation setting, we similarly define 
\begin{align}
\labelAcq[\Ndesign]{\fwdintExpVarLabel}(\designBatchIn) \Def
\int_{\parSpace} \E_{\designBatchFwd} \Var[\fwdEmRdm[\Ndesign]{\postDens}(\Par) | \fwdEm[\Ndesign](\designBatchIn) = \designBatchFwd]\weightDens(\Par) d\Par, \label{fwd-int-exp-var}
\end{align}
...
\begin{enumerate}
\item $\E \left[\Exp{\llikEmFwd[\Ndesign](\Par)} | \fwdEm[\Ndesign](\designBatchIn) = \designBatchFwd \right] = \Gaussian(\obs | \emMean[\Ndesign]{\fwd}(\Par | \designBatchFwd), \CovComb[\Naugment](\Par))$
...
\todo: fix formatting and need to clear up some ambiguous points in the notation; is it clear the first expectation is wrt the emulator? 
And need to make clear in the notation that the G emulator is being plugged-in. 
