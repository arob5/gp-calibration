...
\begin{align}
\funcEm[\Ndesign] \Def \func|\func(\designIn) \sim \GP(\gpMean[\Ndesign], \gpKer[\Ndesign]),
\end{align}
...
$\Em[\Ndesign]{\likRatio}(\Par, \propPar)$. Generalizing to the $\Em[\Ndesign]{\accProbRatio}(\Par, \propPar)$
case is trivial, given that $\Em[\Ndesign]{\accProbRatio}(\Par, \propPar)$ is of the form 
$C \times \Em[\Ndesign]{\likRatio}(\Par, \propPar)$, where $C$ is a constant with respect to $\llikEm$. We thus 
focus on the latter case, defining 
\begin{align}
\llikEmJointMarg[\Ndesign]{\accProbMH}(\Par, \propPar) 
&\Def \E_{\llikEm[\Ndesign]} \left[\max\left\{1, \Em[\Ndesign]{\accProbRatio}(\Par, \propPar)\right\} \right].
\label{acc_prob_joint_marg}
\end{align} 
The expectation in \ref{acc_prob_joint_marg}
is of the form $\E\left[\max\left\{1, Y \right\}\right]$ where $Y$ is log-normally distributed, and hence can be written (almost)
