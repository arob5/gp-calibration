...
Then for an arbitrary set of inputs $\parMat$, the predictive mean $\gpMean[\Naugment](\parMat)$
(viewed as a function of the random variable $\designBatchFunc$), satisfies 
\begin{align}
\gpMean[\Naugment](\parMat)
\sim \Gaussian\left(\E[\funcEm[\Ndesign](\parMat) | \gpMean[\Ndesign](\designBatchIn)], 
                               \gpKer[\Ndesign](\parMat) - \gpKer[\Naugment](\parMat) \right) \label{pred-mean-dist}
\end{align}
\end{lemma}
We see that the mean (with respect to $\designBatchFunc$) of $\gpMean[\Naugment](\parMat)$ is equal to the mean of
the conditional 
...
The variance of 
$\gpMean[\Naugment](\parMat)$ is equal to the change in GP variance at the inputs $\parMat$ due to the act 
of conditioning on the batch $\designBatchIn$. Therefore, the uncertainty in $\gpMean[\Naugment](\parMat)$ 
