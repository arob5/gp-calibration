...
of two Gaussian densities (see \Cref{Gaussian_convolution}). 
...
\item $\E_{\designBatchFwd} \left[\Gaussian(\obs | \emMean[\Naugment]{\fwd}(\Par; \designBatchFwd), \CovComb[\Naugment](\Par))^2\right] $
\end{enumerate}
\end{lemma}
...
&= \Gaussian\left(\obs | \emMean[\Ndesign]{\fwd}(\Par), \CovComb[\Naugment](\Par) + \emKer[\Ndesign]{\fwd}(\Par) - \emKer[\Naugment]{\fwd}(\Par)\right), 
\end{align*}
where the final line uses the Gaussian convolution formula in \Cref{Gaussian_convolution}. 
