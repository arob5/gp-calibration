...
values $\state(\Time, \Par)$ at a finite set of time points $\{0, 1, \dots, \NTimeStep\} \subset [\timeStart, \timeEnd]$. 
...
\begin{align}
\obs &= \fwd(\Par) + \noise \Def (\obsOp \circ \solutionOp)(\Par) + \noise \nonumber \\
\noise &\sim \Gaussian(0, \likPar) \nonumber 
\end{align}
for some parameter value in the support of the prior distribution. Some of the methods discussed below readily 
...
&\Def \left(\sum_{\timeIndex=1}^{\NTimeStep} \left[\state^{(\stateIndex)}_{\timeIndex} \right]^{\idxObsDim} \right)_{\idxObsDim=1}^{\dimObs} \label{obs_op_moments} \\ 
\obsOp(\indexState[0]{\state}, \dots, \indexState[\dimState]{\state}) 
...
\begin{figure}
     ...
     \end{subfigure}
        ...
        with $\fwd(\Par) = \sin(\Par) + 0.1 \Par$ and a single observation $\obs = 0$. 
        ...
        are identical. The dashed black line is the ground truth forward model or (log) likelihood and the red points are the 
        design points used for emulator training. The blue line is the mean of the respective emulators, and the gray area 
        covers the region within one standard deviation of the mean.}
...
implicitly defined. \Cref{fig:post_approx_1d} provides a one-dimensional illustration of the posterior approximations 
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.48\textwidth} % Posterior approximations [fwd]
         ...
         \label{fig:three sin x}
     \end{subfigure}
        \caption{Posterior density approximations derived from the emulators in \Cref{fig:em_dist_1d}. The left and right 
        ...
        interpolate the design points due to differences in normalizing constants. The mean approximation in the 
        forward model emulation Gaussian setting is an exception in which the normalizing constants align.}
...
avenue for inference \cite{pseudoMarginalMCMC}. \Cref{fwd_em_lik_emulator_moments_Gaussian} provides the 
...
\todo: add proof to appendix and define notation used above. 
