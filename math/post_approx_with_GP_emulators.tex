...
In applications, the GP emulator is typically constructed sequentially. \todo: finish intro here 
...
\end{align}
where the predictive mean $\gpMean[\Naugment]$ and covariance $\gpKer[\Naugment]$ are given by \ref{kriging_eqns}
...
\textit{sequential design} (also called \textit{active learning}) loop, summarized in \Cref{alg:seq_des_loop}.  
...
However, computing resources often allow $\BigO(100)$ function evaluations to be performed in parallel. Such constraints imply a 
...
of optimization. The field of \textit{Bayesian optimization} is dedicated to this question, resulting in a wide variety of 
proposed acquisition functions that are tailored to the task of optimizing expensive black-box functions $\func$; 
see \cite{gramacy2020surrogates} and [\todo: include other citation] textbook treatments. 
The related field of \textit{Bayesian quadrature} \cite{BayesQuadrature, BayesQuadRatios} [\todo: other references]
instead focuses on the goal of approximating integrals with expensive integrands. The computer experiments community 
focuses on various tasks, including contour estimation [\todo: cite] and sensitivity analysis [\todo: cite]. However, perhaps the 
most common goal considered in the sequential design of computer experiments is that of filling the input space $\parSpace$. 
\todo: the below designs are not necessarily space-filling (depends on the kernel); should make this more clear. Perhaps 
it is more clear to say that these all target GP predictions over all of the parameter space, which often tends to have a space-filling 
effect. 
Such \textit{space-filling} designs seek to spread the design points ``evenly'' over $\parSpace$ in some specific sense. 
To this end, it seems reasonable to choose points in locations where the GP is uncertain. Defining ``uncertainty'' in terms 
...
&&\labelAcq[\Ndesign]{\maxentLabel}(\Par) \Def -\Ent[\funcEm[\Ndesign](\Par)], \label{acq_max_var_ent}
\end{align} 
where $\Ent[Z]$ denotes the entropy of a random variable $Z$. The variance and entropy are negated to align with our convention 
of minimizing acquisition functions. The acquisition functions \ref{acq_max_var_ent} are defined to take a single design point as 
input, but they can be generalized to operate in the batch setting (\todo: look into this, I think the generalization of maxent leads 
to the determinant of the covariance matrix; for maxvar is there a generalization?). A noted weakness of these criteria is their locality; 
they target the single point with maximal uncertainty, ignoring the effect of the acquisition on the remainder of the design space. 
This often favors points at the boundaries of $\parSpace$, behavior that is often not desirable \cite{gramacy2020surrogates}. 
Addressing this weakness, an alternative approach is to minimize the \textit{conditional} uncertainty 
...
a probability density on $\parSpace$. Since $\Var[\funcEm[\Ndesign](\Par) | \designBatchIn] = \gpKer[\Naugment](\Par)$ and 
...
$\weightDens$ (\todo: cite), the integrals are typically replaced with Monte Carlo approximations. It is worth noting that 
...
(see \ref{kriging_eqns}). A nice summary and analysis of integrated variance criteria of the form \ref{acq_int_var} is given 
...
Then for an arbitrary set of inputs $\parMat$, the predictive mean $\emMean[\Naugment]{\llik}(\parMat)$
(viewed as a function of the random variable $\designBatchLlik$), satisfies 
\begin{align}
\emMean[\Naugment]{\llik}(\parMat) 
\sim \Gaussian\left(\E[\llikEm[\Ndesign](\parMat) | \emMean[\Ndesign]{\llik}(\designBatchIn)], 
                               \emKer[\Ndesign]{\llik}(\parMat) - \emKer[\Naugment]{\llik}(\parMat)] \right) \label{pred-mean-dist}
\end{align}
\end{lemma}
We see that the mean (with respect to $\designBatchLlik$) of $\emMean[\Naugment]{\llik}(\parMat)$ is equal to the mean of
$\llikEm[\Ndesign](\parMat) | (\designBatchIn, \emMean[\Ndesign]{\llik}(\designBatchIn))$, which treats the current 
predictive mean $\emMean[\Ndesign]{\llik}(\designBatchIn)$ as if it were the true response. The variance of 
$\emMean[\Naugment]{\llik}(\parMat)$ is equal to the change in GP variance at the inputs $\parMat$ due to the act 
of conditioning on the batch $\designBatchIn$. Therefore, the uncertainty in $\emMean[\Naugment]{\llik}(\parMat)$ 
is large when the design points $\designBatchIn$ are more ``influential''. The above lemma allows us to 
