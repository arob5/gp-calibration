...
\todo: maybe insert this sentence somewhere above: 
...
achieve such goal-oriented sequential design. We start by focusing on the log-likelihood emulation setting, with 
the analogous quantities for forward model emulators given in \todo. 
...
$\funcEm$ with $\llikEm$ in the definitions. However, note that the resulting $\maxvarLabel$ and 
...
\end{equation}
[\todo: include the $\maxexpentLabel$ criterion here as well; need to consider how to present this; formula is a bit 
messier in the batch setting due to the multivariate lognormal expressions.] These criteria strike a balance between 
$\emMean[\Ndesign]{\llik}(\Par)$, $\emKer[\Ndesign]{\llik}(\Par)$, and $\priorDens(\Par)$, rather than only considering 
$\emKer[\Ndesign]{\llik}(\Par)$. However, they still suffer from the drawbacks associated with local uncertainty criteria, 
...
We start by setting $\qoiMap$ as in \ref{qoi_exp_var}, which results in an integrated variance conditional 
...
provides a closed-form expression for the expected variance expression \ref{acq_int_cond_unc}. See 
...
also closely follows the above discussion of this lemma. In particular, the variance term in the integrand is
the variance of the log-normal random variable $\llikEmRdmDens[\Ndesign](\Par)$ 
...
In adapting local uncertainty criteria \ref{acq_local_unc} to the goal of posterior approximation, 
$\E\left[\qoi(\designBatchIn) \right]$
