...
the standard for uncertainty quantification in such contexts, but challenges persist in adapting statistical inference techniques
...
the likelihood implies performing a complex computer simulation; e.g., \citep{ESM_modeling_2pt0}. 
...
parallel), which can then replace the exact likelihood to facilitate the application of 
standard inference schemes such as Markov chain Monte Carlo (MCMC).

The present work is focused on the use of \textit{stochastic} emulators to facilitate Bayesian inversion for parameters of interest.
...
Probabilistic surrogates may arise when a parametric emulator model is fit using Bayesian methods; in this case the 
emulator uncertainty may be encoded by a collection of samples summarizing the posterior over the (finite) set of 
emulator parameters. In an alternative non-parametric approach, the emulator may explicitly model the true likelihood 
as a random function. Gaussian processes (GPs; \citet{gpML}) are the canonical example of the latter method, and
have been widely used as emulators in calibrating complex computer codes 
\citep{design_analysis_computer_experiments,SanterCompExp,gramacy2020surrogates}. A growing body of research 
has begun to focus on the use of GP emulators in constructing posterior approximations for Bayesian inverse problems
...
The focus on GPs facilitates analytical computations 
...
severely limits modeling flexibility. For example, the approaches in many of the references cited above are limited to 
GP surrogates with fixed hyperparameters, Gaussian likelihoods, or both. Any deviation from these assumptions breaks 
the analytical tractability enjoyed in this limited setting.

We begin by reviewing the literature focused on solving Bayesian inverse problems with surrogate models,
drawing a distinction between two general strategies that are common in the literature: (i.) forward model emulation, whereby
(some function of) the expensive computer model is approximated by an emulator, and (ii.) log-likelihood emulation, whereby
the log-likelihood is emulated directly. The latter is a special case of the former, and both approaches ultimately induce an 
approximation of the likelihood function, and thus the posterior distribution as well. However, it is still useful to make this distinction, 
...
We next proceed to review methods of posterior approximation using probabilistic surrogate models. We begin with a thorough 
review of the GP setting, where a variety of approximations have been proposed across several different literatures; 
...
approaches. We then proceed to describe generalizations to non-Gaussian stochastic emulators, as well as recent work
that seeks to develop a framework for posterior approximation that is applicable to generic probabilistic surrogate models.
Of particular note is the work \citet{BurknerSurrogate}, which investigates several methods that can be viewed as 
Monte Carlo approximations of approaches that have been utilized in the GP emulation literature. 
We also discuss [our work], which provides an alternative approach to generate approximate posterior samples using a 
stochastic surrogate, with the additional benefit of handling the infinite-dimensional non-parametric case.

Finally, we turn to the problem of sequential design for surrogate models with respect to the goal of improving the 
current posterior approximation.


[\todo: it is probably worth stressing somewhere that the analytical GP results are quite restrictive; e.g., imposing bound 
constraints, Bayesian treatment of hyperparameters, or the use of more flexible models (e.g., deep GPs) all result
in the loss of closed-form computations. Motivation to develop methods that only require sampling from the emulator
predictive distribution. Maybe even worth re-framing the paper in terms of "probabilistic surrogate models" instead of 
specifically GPs]
...
using a new set of exact simulation evaluations. Choosing the new set of parameter values at which to evaluate
the simulator is a question of sequential design, also referred to as active learning.
...
\citep{BayesQuadrature,BayesQuadratureAL,BayesQuadRatios}, but in this case the goal is to estimate a posterior density.
...
[\todo: note that I drafted the above introduction with a single paper in mind that is part review paper, part our new proposed
methods. I'm sticking in this section here to provide a more in depth review of past work, with no mention of our proposed
methods. There is overlap with the intro, but just sticking this here for now.]
