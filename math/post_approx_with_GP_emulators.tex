...
multi-output GP prior directly for the forward model, $\fwd \sim \GP(\emMean{\fwd}, \emKer{\fwd})$. After 
...
the predicted forward model outputs at inputs $\parMat$ is summarized by the entire distribution 
...
\begin{align}
\llikEmFwd[\Ndesign](\Par) := \log\Gaussian(\obs| \fwdEm[\Ndesign](\Par), \likPar). \label{induced_llik_emulator}
\end{align}
We emphasize two facts: (i.) the induced log-likelihood approximation $\llikEmFwd[\Ndesign](\Par)$ is also 
stochastic, but no longer Gaussian; and (ii.) $\llikEmFwd[\Ndesign]$ is a ``plug-in'' estimator for $\llik$; no 
prior model for $\llik$ is specified. Instead, a modular approach \cite{modularization} is taken in which all of 
the surrogate modeling decisions are made for $\fwd$, and then the resulting emulator $\fwdEm[\Ndesign]$ 
is simply plugged into the likelihood. Emulating expensive models using GPs is one of the central questions 
...
a differential equation and thus may take the form of a long time series or large spatiotemporal grid. 
...
emulation in this paper are easily extended to this setting. See the appendix for details (TODO). 
...
The emulator construction proceeds as before; a GP prior $\llik \sim \GP(\emMean{\llik}, \emKer{\llik})$ is specified and the predictive 
...
Plugging the log-likelihood surrogate into the posterior density expression \ref{post_dens} yields 

\begin{align}
\emApprox[\Ndesign]{\postDensNorm}(\Par) := \frac{1}{\emApprox[\Ndesign]{\normCst}}\emApprox[\Ndesign]{\postDens}(\Par) 
:= \frac{1}{\emApprox[\Ndesign]{\normCst}} \priorDens(\Par) \Exp{\llikEmGeneric[\Ndesign](\Par)}, \label{post_dens_random}
\end{align}
with normalizing constant given by 
\begin{align}
\emApprox[\Ndesign]{\normCst} &= \int \priorDens(\Par) \Exp{\llikEmGeneric[\Ndesign](\Par)} d\Par. \label{norm_cst_random}
\end{align}
The unnormalized posterior density $\emApprox[\Ndesign]{\postDens}$ is stochastic; each sample trajectory from the 
underlying GP induces a different posterior. The papers \cite{StuartTeck1, StuartTeck2} refer to this as the \textit{sample-based}
approximation of the posterior. Given that it is not obvious how to perform inference using this random object, one typically 
proceeds by choosing a single, deterministic posterior approximation that somehow summarizes the uncertainty in the 
random posterior measure. The popular choices for such deterministic approximations are presented below. 

\subsection{Deterministic Approximations}
Arguably the simplest approach is to reduce the random emulator to a deterministic one by considering only the GP 
predictive mean. The plug-in mean estimators yield posterior approximations with unnormalized densities 
\begin{align}
\emPostDensMean[\Ndesign](\Par) &= \priorDens(\Par) \Exp{\emMean[\Ndesign]{\llik}(\Par)}
\end{align}
