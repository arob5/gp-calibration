...
The emulator can be trained offline using a small set of expensive model runs, which can often be performed in 
parallel. The exact likelihood is then replaced by the fit surrogate in order to facilitate the application of standard inference 
schemes such as Markov chain Monte Carlo (MCMC). 

In the present work we focus on the use of Gaussian processes (GPs) for this task, which have been widely used as 
emulators in calibrating complex computer codes. GP predictions come in the form of a probability distribution, implying that 
...
some convenient closed-form computations. A growing number of papers have also begun to explore the latter option. 
...
to illustrate the implications of this choice in defining a GP-induced posterior approximation. 
...
focusing the model evaluation locations in regions of significant posterior probability. Casting the selection of 
...
(an exploration vs. exploitation tradeoff). This sequential optimization framework bears close resemblance to related 
topics such as Bayesian optimization and Bayesian quadrature, but in this case the goal is to estimate a posterior density.
...
to parameter estimation for systems of ordinary differential equations. In particular, we provide numerical results for 
parameter calibration problems for ecological models of the carbon cycle.
