...
$\emLaw[{\design[\Naugment]}]{\func} = \emLaw[{\design[\Nbatch]}]{\funcEm}$, and is justified in \Cref{lemma:gp-condition-order}. 
...
\begin{align}
\E_{\designBatchLlik} \Var[\llikEmRdmDens[\Naugment](\Par; \designBatchLlik)]
...
\int_{\parSpace} \E_{\designBatchLlik} \Var[\llikEmRdmDens[\Naugment](\Par; \designBatchLlik)] \weightDens(\Par) d\Par, 
\end{align}
...
\int_{\parSpace} \E_{\designBatchLlik} \Var[\fwdEmRdm[\Naugment]{\postDens}(\Par; \designBatchLlik)] \weightDens(\Par) d\Par. \label{fwd-int-exp-var}
\end{align}
To our knowledge, 
...
Then for an arbitrary set of inputs $\parMat$, the predictive mean $\emMean[\Naugment]{\fwd}(\parMat; \designBatchFwd)$
(viewed as a function of the random variable $\designBatchFwd$), satisfies 
\begin{align}
\emMean[\Naugment]{\fwd}(\parMat; \designBatchFwd) 
