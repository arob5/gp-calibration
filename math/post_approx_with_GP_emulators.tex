...
(surrogate refinement via the selection of additional design points is the topic of  \Cref{seq_des}). It is not difficult to 
...
(PAR; \textrm{MJ/$m^2$/day}), and the above dynamics rely on the following parameterized model of 
...
\end{align} 
We note that the ODE is of the form \ref{ode_ivp}, where the nonlinear dynamics $\odeRHS$ additionally depend on a 
...
\end{align*}
The single observation $\obs = $ was obtained simulated using this data-generating process with input parameter $\ParTrue$ 
