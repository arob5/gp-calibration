...
We begin by reviewing the literature focused on GP-induced posterior approximation. A variety of approximations have been 
proposed across several disciplines; we collect these results here, presenting streamlined derivations aimed at clarifying the 
assumptions underlying the various approaches. 

\citep{BurknerSurrogate}


GP predictions come in the 
form of a probability distribution, implying that the emulator induces a stochastic approximation of the likelihood function. 
The fact that the surrogate likelihood is a random 
...
approximate inference algorithms based on noisy modifications of Metropolis-Hastings MCMC schemes. Restricting the 
focus to GP emulators allows for many analytical calculations, which allows for interpretable methods for surrogate 
uncertainty propagation and sequential design. However, our noisy MCMC framework is well-suited to handle 
surrogate models with non-Gaussian predictive distributions. 
