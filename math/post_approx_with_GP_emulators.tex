...
$\func(\parMat) \Def [\func(\Par_1), \dots, \func(\Par_\Ndesign)]^\top \in \R^{\Ndesign}$, 
...
\end{align}
Now suppose that, at a set of \textit{design points} $\designIn$, we have observed the (noiseless) function evaluations 
$\funcValExact$; i.e., a realization of $\func(\designIn)$. 
It is well-known \cite{gpML} that the conditional distribution $\func|[\func(\designIn)=\funcValExact]$ is also a GP
\begin{align}
\funcEm[\Ndesign] \Def \func|[\func(\designIn)=\funcValExact]\sim \GP(\gpMean[\Ndesign], \gpKer[\Ndesign]), \label{generic_gp_conditional}
\end{align}
with mean and kernel given by 
\begin{align}
\gpMean[\Ndesign](\parMat) &= \gpMeanPrior(\parMat) + \gpKerPrior(\parMat, \designIn) \gpKerPrior(\designIn)^{-1} [\funcValExact - \gpMeanPrior(\designIn)] \label{kriging_eqns} \\
\gpKer[\Ndesign](\parMat) &= \gpKerPrior(\parMat) - \gpKerPrior(\parMat, \designIn) \gpKerPrior(\designIn)^{-1} \gpKerPrior(\designIn, \parMat). \nonumber
\end{align}
The predictive mean can be seen to interpolate the design points (i.e., $\gpMean[\Ndesign](\designIn) = \funcValExact$), with 
...
runs, which can often be computed in parallel, to obtain a design $\{\designIn, \designOutFwdExact\}$ or 
$\{\designIn, \designOutLlikExact\}$ which can then be used to train a model that 
approximates $\fwd$ or $\llik$. Here, $\designOutFwdExact$ and $\designOutLlikExact$ denote the vectors containing the 
$\Ndesign$ forward model and log-likelihood evaluations at inputs $\designIn$, respectively. 
...
multi-output GP prior directly for the forward model, $\fwd \sim \GP(\emMeanPrior{\fwd}, \emKerPrior{\fwd})$. 
Conditioning on this design yields the predictive distribution 
$\fwdEm[\Ndesign]  \Def \fwd|[\fwd(\designIn) = \designOutFwdExact] \sim \GP(\emMean[\Ndesign]{\fwd}, \emKer[\Ndesign]{\fwd})$, 
...
\begin{align*}
\llikEm[\Ndesign] \Def \llik | [\llik(\designIn) = \designOutLlikExact[\Ndesign]] \sim \GP(\emMean[\Ndesign]{\llik}, \emKer[\Ndesign]{\llik})
\end{align*}
is obtained by conditioning on the design $\{\designIn, \designOutLlikExact[\Ndesign]\}$. 
...
of two Gaussian densities (see \Cref{Gaussian_convolution} in the appendix). 
...
values are sampled independently across iterations, the \textit{MCWMH-joint} does indeed define a valid 
...
for a generic GP predictive distribution $\funcEm[\Ndesign] \Def \func|[\func(\designIn) = \funcValExact]$, as defined in 
\ref{generic_gp_conditional}. We now consider the question of identifying a new set of $\Nbatch$
inputs $\designBatchIn$ to augment the initial design. We let $\funcValExact[\Nbatch]$ denote the true, noiseless 
function evaluations at $\designBatchIn$. The augmented design is thus given by 
$\{\designIn[\Naugment], \funcValExact[\Naugment]\}$, where 
$\designIn[\Naugment] \Def \designIn[\Ndesign] \cup \designBatchIn$ and $\funcValExact[\Naugment]$ is 
the observed realization of $\func(\designIn[\Naugment])$. 
Conditioning on the augmented design results in the updated GP
\begin{align}
\funcEm[\Naugment] \Def \func|[\func(\designIn[\Naugment]) = \funcValExact[\Naugment]] \sim \GP(\gpMean[\Naugment], \gpKer[\Naugment]),
\end{align}
where the predictive mean $\gpMean[\Naugment]$ and covariance $\gpKer[\Naugment]$ functions are given by \ref{kriging_eqns}
with $\{\designIn, \funcValExact[\Ndesign]\}$ replaced by $\{\designIn[\Naugment], \funcValExact[\Naugment]\}$. 
We note that conditioning in stages is equivalent to conditioning on the entire design at once; i.e.,
$\func|[\func(\designIn[\Naugment]) = \funcValExact[\Naugment]] \eqDist 
\funcEm[\Ndesign]|[\funcEm[\Ndesign](\designIn[\Nbatch]) = \funcValExact[\Nbatch]]$, which is justified 
...
while the design framework summarized in \Cref{alg:seq_des_loop} does allow for consideration of within-batch interactions between design points, 
...
We define 
\begin{align}
\llikEmRdmDens[\Naugment](\Par; \designBatchLlik) \Def \llikEmRdmDens[\Ndesign](\Par) | \designRand{\designBatchLlik}
\end{align}
\todo: need to be more precise here.  
...
\int_{\parSpace} \E_{\designBatchFwd} \Var[\fwdEmRdm[\Naugment]{\postDens}(\Par; \designBatchFwd) | \designBatchFwd] \weightDens(\Par) d\Par. \label{fwd-int-exp-var}
\end{align}
To our knowledge, 
...
We begin with a lemma that will be used in the proof of \Cref{lemma:fwd-Gaussian-density-dist} below. 
