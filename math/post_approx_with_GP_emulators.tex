...
solution is to extend the input space of the emulator to include the likelihood parameters. This is explored in 
\cite{llikRBF}. Finally, it may be difficult to incorporate prior domain knowledge 
...
\max\left\{1,  \frac{\priorDens(\propPar) \propDens(\propPar, \Par)}
{\priorDens(\Par)  \propDens(\Par, \propPar)} \accProbRatio(\Par, \propPar)  \right\}, 
&&\accProbRatio(\Par, \propPar) = \frac{\Exp{\llik(\propPar)}}{\Exp{\llik(\Par)}}.
...
We refer to $\accProbRatio(\Par, \propPar)$ and $\accProbMH(\Par, \propPar)$ as the likelihood 
ratio and acceptance probability, respectively. 
...
\begin{algorithm}
    ...
    \Function{MH}{$\indexMCMC[0]{\Par}, \NMCMC$}     
		...
		\State $\accProbMH(\indexMCMC{\Par}, \propPar) \gets \max\left\{1, \frac{\priorDens(\propPar)\Exp{\llik(\propPar)}
...
or $\llikEmFwd[\Ndesign]$ to approximate $\accProbRatio(\Par, \propPar)$ or $\accProbMH(\Par, \propPar)$. 
...
We thus consider the random variables $\Em[\Ndesign]{\accProbRatio}(\Par, \propPar)$ and 
...
section correspond to the following deterministic, plug-in estimates of $\accProbRatio(\Par, \propPar)$:
\begin{align}
\llikEmMean[\Ndesign]{\accProbRatio}(\Par, \propPar) 
:= \Exp{\emMean[\Ndesign]{\llik}(\propPar) - \emMean[\Ndesign]{\llik}(\Par)} \\
\llikEmMarg[\Ndesign]{\accProbRatio}(\Par, \propPar) 
:= \llikEmMean[\Ndesign]{\accProbRatio}(\Par, \propPar) 
\Exp{\frac{1}{2}\left[\emKer[\Ndesign]{\llik}(\propPar) - \emKer[\Ndesign]{\llik}(\Par)\right]} \\
\llikEmQ[\Ndesign]{\accProbRatio}(\Par, \propPar) 
:= \llikEmMean[\Ndesign]{\accProbRatio}(\Par, \propPar) 
...
by marginalizing $\Em[\Ndesign]{\accProbRatio}(\Par, \propPar)$ with respect to the \textit{joint} 
distribution of $[\llikEm[\Ndesign](\propPar), \llikEm[\Ndesign](\Par)]$, which gives 
\begin{align}
\llikEmJointMarg[\Ndesign]{\accProbRatio}(\Par, \propPar) 
&:= \E_{\llikEm[\Ndesign]} \left[\Em[\Ndesign]{\accProbRatio}(\Par, \propPar) \right] \\
&= \llikEmMean[\Ndesign]{\accProbRatio}(\Par, \propPar)  \Exp{\frac{1}{2} \Var\left[\llikEm[\Ndesign](\propPar) - \llikEm[\Ndesign](\Par) \right]} \\
&=  \llikEmMean[\Ndesign]{\accProbRatio}(\Par, \propPar)  
\Exp{\frac{1}{2}\left[\emKer[\Ndesign]{\llik}(\propPar) + \emKer[\Ndesign]{\llik}(\Par) - \emKer[\Ndesign]{\llik}(\propPar, \Par) \right]}. 
...
We conclude this section by noting that the joint marginal approximation can be taken one step further by considering 
\begin{align}
\llikEmJointMarg[\Ndesign]{\accProbMH}(\Par, \propPar) 
&= \E_{\llikEm[\Ndesign]} \left[\max\left\{1, \Em[\Ndesign]{\accProbRatio}(\Par, \propPar)\right\} \right],
\label{acc_prob_joint_marg}
\end{align} 
whereby the expectation has been moved outside of the maximum. The expectation in \ref{acc_prob_joint_marg}
is of the form $\E \max\left\{1, Y \right\}$ where $Y$ is log-normally distributed, and hence can be written (almost)
in closed form with respect to elementary functions and the Gaussian distribution function. See the appendix for 
details. \todo
...
\begin{algorithm}
    ...
    \Function{noisyMH}{$\indexMCMC[0]{\Par}, \NMCMC, \propDens, \llikSampDist$}     
		...
		\State $\accProbMH(\indexMCMC{\Par}, \propPar) \gets \max\left\{1, \frac{\priorDens(\propPar)e^{\llikSampProp} 
...
We define the following noisy MCMC algorithms based on different choices for $\llikSampDist_{\Par, \propPar}$. Letting
$\parMat := \{\Par, \propPar\}$, we consider the following choices. 
...
+ \delta_{\Par}(A) \left[1 - \int_{A} \propDens(\Par, \propPar) \llikEmJointMarg[\Ndesign]{\accProbMH}(\Par, \propPar) d\propPar \right]
\label{MCWMH-joint-kernel}
\end{align}
which is derived in the appendix (\todo). This is precisely the transition kernel of the Markov chain defined by the deterministic 
approximation joint marginal approximation of the acceptance probability \ref{acc_prob_joint_marg}. 
...
= \Gaussian(\emMean[\Ndesign]{\llik}(\parMat), \text{diag}\left\{\emKer[\Ndesign]{\llik}(\Par), \emKer[\Ndesign]{\llik}(\propPar)\right\}).
\label{llik_dist_MCWMH-ind}
\end{align}
We refer to the resulting algorithm as \textit{Independent Monte Carlo Within Metropolis-Hastings (MCWMH-ind)}. 
...
\subsection{Transition Kernels of Approximate MCMC Algorithms}
...
\end{align}
Denote $\parMat := \{\Par, \propPar\}$. 
The \textit{MCWMH-joint} algorithm replaces the exact log-likelihood evaluations 
$\llik(\parMat) := [\llik(\Par), \llik(\propPar)]^\top$ used to define $\propDens(\Par, \propPar)$ by 
sampled approximate values 
\begin{align}
\llikSamp_{\parMat} := [\llikSamp, \llikSampProp]^\top \sim \Gaussian(\emMean[\Ndesign]{\llik}(\parMat), \emKer[\Ndesign]{\llik}(\parMat)). 
\end{align}
...
in \ref{acc_prob_joint_marg}. Setting $A := \parSpace$ in the above integral yields the overall acceptance probability 
\begin{align}
\llikEmJointMarg[\Ndesign]{\avgAccProbMH}(\Par) := \int_{\parSpace} \propDens(\Par, \propPar) \llikEmJointMarg[\Ndesign]{\accProbMH}(\Par, \propPar)  d\propPar. 
\end{align}
