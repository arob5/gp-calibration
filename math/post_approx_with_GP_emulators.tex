...
\label{acc_prob_joint_marginal}
\end{align} 
whereby the expectation has been moved outside of the maximum. The expectation in \ref{acc_prob_joint_marginal}
...
We define the following noisy MCMC algorithms based on different choices for $\llikSampDist_{\Par, \propPar}$. Let
$\parMat := \{\Par, \propPar\}$, we consider: 
\begin{enumerate}
\item Joint Monte Carlo Within Metropolis-Hastings (\textbf{MCWMH-joint}): 
\begin{align}
\llikSampDist_{\Par, \propPar} = \Gaussian(\emMean[\Ndesign]{\llik}(\parMat), \emKer[\Ndesign]{\llik}(\parMat))
\end{align}
\item Independent Monte Carlo Within Metropolis-Hastings (\textbf{MCWMH-ind})
\begin{align}
...
\end{align}
\item Pseudo-Marginal (\textbf{pseudo-marg})
\begin{align}
\llikSampDist_{\Par, \propPar} 
= \delta_{\Par} \otimes  
\Gaussian(\emMean[\Ndesign]{\llik}(\propPar), \emKer[\Ndesign]{\llik}(\propPar)).
\end{align}
\end{enumerate}

Note that we have continued to specialize the definitions to the log-likelihood emulation setting, but the forward model 
