...
Sinsbeck and Nowak propose a sequential design method which constructs and refines a deterministic approximation to the true likelihood $\mathcal{L}(\btheta)$ at each iteration of the sequential design procedure. 
...
\[\ell(\mathcal{L}, \mathcal{L}^\prime) := \E_{\btheta \sim \pi_0}\left[\left(\mathcal{L}(\btheta) - \mathcal{L}^\prime(\btheta) \right)^2 \right]\]
...
\[\mathcal{R}(\mathcal{L}_n^*, \mathcal{L}_n) := \E_{\Phi^*} \E_{\pi_0} \left[\left(\mathcal{L}^*(\btheta) - \mathcal{L}(\btheta) \right)^2 \right] \]
...
\begin{align}
\mathcal{R}(\mathcal{L}_n^*, \hat{\mathcal{L}}_n) = \E_{\pi_0} \Var_{\Phi^*}\left[\mathcal{L}_n^*(\btheta)\right] \label{Bayes_Risk}
\end{align}
...
$(\tilde{\btheta}, \tilde{\Phi})$ and condition on this additional observation. Recall that $\mathcal{R}(\mathcal{L}^*_n, \hat{\mathcal{L}}_n)$ denotes the Bayes' risk between the 
...
With this notation established, $\mathcal{R}(\tilde{\mathcal{L}}^*_n, \hat{\mathcal{L}}_n)$ is an estimator for the future Bayes' risk, were we 
...
\begin{align}
J_n(\tilde{\btheta}) &:=  \E_{\tilde{\Phi}} \mathcal{R}(\tilde{\mathcal{L}}^*_n, \hat{\mathcal{L}}_n) \\ 
			     &= \E_{\tilde{\Phi}} \E_{\pi_0} \Var_{\tilde{\Phi}_n^*}\left[\tilde{\Phi}_n^*(\btheta) \right]
\end{align}
