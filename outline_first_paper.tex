...
If we consider discretizing at constant time steps $\Delta t$ then since the system is autonomous then the \textit{one-step map} or \textit{flow map} $g: \R^K \to \R^K$ is time-independent: 
 ...
 \]
 where I now explicitly write $G$ as a function of an initial condition $\bx_0 = \bx(0)$, which is independent of $\btheta$, and I use the shorthand $g_\theta^{(t)}$ to denote 
 the composition consisting of $t$ applications of the map $g_\theta$. I have also assumed that the initial time is $0$, but 
 some other time $t_0$ could of course be considered. 
 
\cite{MOHAMMADI2019178}
