...
\begin{align*}
K = \nu C = \nu \left(C_0 + \tau^2 I_N \right)
\end{align*}
where $C_0$ has entries given by the squared exponential correlation function 
\begin{align*}
\left[C_0\right]_{ij} = \exp\left\{-\sum_{d = 1}^{D} \frac{((x_i)_d - (x_j)_d)^2}{\theta_d} \right\}
\end{align*}
...
I now compute the gradient of the log-likelihood with respect to $\phi = (\theta, \tau^2, \nu)$. The following facts will be helpful. 
\begin{align*}
...
\end{align*}
Also note that $\theta_d$, $\tau^2$, $\nu$ are all constrained to be positive. The two main options for accounting for this constraint in the optimization are to 1.) parameterize the 
objective function in terms of the log of these parameters or 2.) define bounds explicitly in the optimization (if allowed in the specific optimization algorithm). For now I consider the 
latter in order to align with the \textit{hetGP} implementation. Denote $\ell(\phi) := \log p(y|X, \phi)$. For any hyperparameter $\phi_j \in \{\theta, \tau^2\}$ the general form of the gradient is 
\begin{align*}
...
I will consider either defining $\tau$ or $\tau^2$ as the parameter. We have 
\begin{align*}
&\frac{\partial C}{\partial \tau^2} = I_N, &&\frac{\partial C}{\partial \tau} = 2\tau I_N
\end{align*}

\subsubsection{Marginal Variance Gradient}
This derivative is simpler as $\nu$ appears outside of $C$. We have 
\begin{align*}
\nabla_{\nu} \ell(\phi) &= -\frac{N}{2\nu} + \frac{y^\top C^{-1} y}{2\nu^2}
\end{align*}
