...
i.e. the approximation resulting from integrating the GP out of the random field $\mathcal{L}_n^*(\btheta)$. With this approximation, the Bayes' risk reduces to the prior-weighted predictive variance 
of $\mathcal{L}_n^*(\btheta)$: 
\[\mathcal{R}(\mathcal{L}_n^*, \mathcal{L}_n) = \E_{\pi_0} \Var_{\Phi^*}\left[\mathcal{L}_n^*(\btheta)\right]\] 
