...
The traditional statistical approach to modeling a physical process is to assume a data model
\[z = f(\theta) + \epsilon\]
where observations $z$ of the true underlying process $f(\theta)$ are assumed to be corrupted by some random noise $\epsilon \sim N(0, \sigma^2)$. We are typically interested 
in inferring the value of some unobserved parameters $\theta$ given the noisy observations of the process. 
