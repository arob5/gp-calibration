...
Complex computer models have become essential tools for studying complex phenomena in a wide variety of scientific and engineering fields, 
...
unknown ecological parameters such as the soil respiration rate or seasonal leaf growth \cite{Fer}. The task is then to confront these models with observational data in 
order to calibrate the unknown parameters, utilize the calibrated model for prediction, and carefully track sources of uncertainty throughout the analysis. Due to its..., 
these challenges are typically addressed within a Bayesian framework; a simplified example of this approach might look like
\begin{align*}
&y = f(\theta) + \epsilon \\
...
the problem of modeling the terrestrial component of the carbon cycle \cite{Friedlingstein}, which is essential both for long-term climate change forecasts (\textbf{cite}) as well as near-term
