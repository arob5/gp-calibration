...
```{r}
par_true <- inv_prob$par_true
llik_func <- inv_prob$llik_obj$get_llik_func()
llik_true <- llik_func(par_true)
llik_em$sample(par_true)

llik_em$sample(rbind(par_true,par_true))
test <- llik_em$predict(rbind(par_true,par_true), return_cov=TRUE)
test$mean
test$var
test$cov
...
llik_em_copy$predict(par_true)
llik_em_copy$update_emulator(par_true, llik_true)

dim(llik_em$get_design_inputs())
dim(llik_em_copy$get_design_inputs())

dim(llik_em$get_design_llik())
dim(llik_em_copy$get_design_llik())
```

```{r}
mcmc_list <- mcmc_approx_settings$`marg-acc-prob-ind`
mcmc_list$n_itr <- 100
mcmc_list$itr_start <- 50
mcmc_list$n_chain <- 1L
mcmc_list$ic_settings$n_ic_by_method <- c(approx_max=1)

mcmc_output <- run_mcmc(llik_em, par_prior, mcmc_list)

head(mcmc_output$samp)
head(mcmc_output$info)
...
```{r}
summarize_approx_mcmc <- function(test_label, itr_start=1L) {
  
  # Determine burn-in.
  itr_start <- setNames(itr_start, test_label)
  ...
  # MCMC diagnostics.
  trace_plots <- get_trace_plots(samp_dt_approx, itr_start=itr_start)
  llik_trace_plot<- get_trace_plots(mcmc_list$info, param_names="llik", 
                                    itr_start=itr_start)[[1]]
  ...
  hist_plot_llik <- get_hist_plots(mcmc_list$info, param_names="llik",
...
```{r}
summarize_approx_mcmc("mean", itr_start=80000L)
```

# Marginal Posterior

```{r}
summarize_approx_mcmc("marginal", itr_start=80000L)
```
...
## 70th percentile
```{r}
summarize_approx_mcmc("quantile7", itr_start=80000L)
```

## 80th percentile
```{r}
summarize_approx_mcmc("quantile8", itr_start=80000L)
```

## 90th percentile
```{r}
summarize_approx_mcmc("quantile9", itr_start=80000L)
```
...
```{r}
summarize_approx_mcmc("mcwmh-joint", itr_start=80000L)
```

# marg-acc-prob-joint Posterior
```{r}
# summarize_approx_mcmc("marg-acc-prob-joint", itr_start=80000L)
```

# mcwmh-ind Posterior
```{r}
summarize_approx_mcmc("mcwmh-ind", itr_start=80000L)
```

# marg-acc-prob-ind Posterior
```{r}
# summarize_approx_mcmc("marg-acc-prob-ind", itr_start=80000L)
```
