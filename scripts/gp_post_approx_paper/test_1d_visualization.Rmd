...
# TODO: 
#    - Why is fwd model post estimate underpredicting so much? 
#    - Should any of the normalized post approximations interpolate the design points? 
...
llik_exact <- llikEmulatorExactGaussDiag(llik_lbl="exact", fwd_model=fwd_model, 
                                         y_obs=matrix(y), dim_par=1L, use_fixed_lik_par=TRUE,
...
```{r}
# Grid of test points for plotting and emulator evaluation. 
N_grid <- 201
...
post_approx_llik <- apply(post_approx_llik, 2, function(x) normalize_lik(x, dx))
...
post_approx_fwd <- apply(post_approx_fwd, 2, function(x) normalize_lik(x, dx))
...
```{r}
# Sample (approximately) from the sample-based (sb) GP-approximated posterior. We consider this as 
...
post_samples_sb_llik <- sim_sample_based_approx_1d_grid(em_llik, par_prior_params, 
                                                        bounds=prior_support, 
                                                        N_grid=N_grid_sb, N_samp=N_samp_sb)
post_samples_sb_fwd <- sim_sample_based_approx_1d_grid(em_fwd, par_prior_params, 
                                                       bounds=prior_support, 
                                                       N_grid=N_grid_sb, N_samp=N_samp_sb)

# Compute kernel density estimates. 
kde_fit_sb_llik <- kde1d(post_samples_sb_llik, xmin=prior_support[1], xmax=prior_support[2], deg=2, mult=1)
post_approx_llik <- cbind(post_approx_llik, `sample`=dkde1d(test_info$input, kde_fit_sb_llik))

kde_fit_sb_fwd <- kde1d(post_samples_sb_fwd, xmin=prior_support[1], xmax=prior_support[2], deg=2, mult=1)
post_approx_fwd <- cbind(post_approx_fwd, `sample`=dkde1d(test_info$input, kde_fit_sb_fwd))

hist(post_samples_sb_llik,30)
hist(post_samples_sb_fwd,30)
```
