...
plt_gp_dist_llik <- em_llik$emulator_model$plot_pred_1d(test_info$input, include_interval=TRUE,
                                                        ...
                                                        ggformat_journal(xlim=xlim, ylim=ylim)
...
# Constant to scale likelihoods by to produce posterior apoproximations. 
dx <- test_info$input[2,] - test_info$input[1,]
norm_cst <- 0.5 * dx * (exp(test_info$llik[1]) + exp(test_info$llik[N_grid])) + 
                  dx * sum(exp(test_info$llik)[2:(N_grid-1)])
prior_dens <- exp(calc_lprior_theta(test_info$input[1,], inv_prob$par_prior))
scale_cst <- prior_dens / norm_cst


# Log-likelihood emulation. 
lik_approx_llik <- em_llik$calc_lik_approx_comparison(test_info$input, approx_types=c("mean", "marginal"), 
                                                      emulator_pred_list=emulator_pred_llik) * scale_cst
lik_approx_llik_plt <- plot_curves_1d_helper(test_info$input, lik_approx_llik, y_new=exp(test_info$llik)*scale_cst,
                                             X_design=design_info$input, y_design=exp(design_info$llik)*scale_cst,
                                             plot_title="Likelihood Approximations [llik]",
                                             line_thickness=line_thickness, design_color=design_color,
                                             design_pt_size=design_pt_size, xlab="u",  ylab="exp L(u)") %>%
                                             ggformat_journal(legend_position="none", xlim=xlim, ylim=ylim*scale_cst)
...
get_hist_plot_comparisons(samp_dt, burn_in_start=burn_in_start, test_label_baseline="nocov-llik")[[1]]
```
...
# MCWMH-Ind. 
kde_fit_mcwmh_ind <- kde1d(select_mcmc_samp(samp_dt, burn_in_start=burn_in_start, test_labels="nocov-llik")$sample)
lik_approx_llik <- cbind(lik_approx_llik, `mcwmh-ind`=dkde1d(test_info$input, kde_fit_mcwmh_ind))

# MCWMH-Joint. 
kde_fit_mcwmh_joint <- kde1d(select_mcmc_samp(samp_dt, burn_in_start=burn_in_start, test_labels="cov-llik")$sample)
lik_approx_llik <- cbind(lik_approx_llik, `mcwmh-joint`=dkde1d(test_info$input, kde_fit_mcwmh_joint))
...
```{r}
#
# Plot: comparison of likelihood approximations. 
#
...
lik_approx_llik_plt <- plot_curves_1d_helper(test_info$input, lik_approx_llik[,2:4], y_new=exp(test_info$llik),
                                             X_design=design_info$input, y_design=exp(design_info$llik),
                                             plot_title="Likelihood Approximations [llik]",
                                             line_thickness=line_thickness, design_color=design_color,
                                             design_pt_size=design_pt_size, xlab="u",  ylab="exp L(u)") %>%
                                             ggformat_journal(legend_position="none", xlim=xlim, ylim=ylim)
plot(lik_approx_llik_plt)

# TODO: need to normalize differently for each approximation. 
