...
llik_bounds <- llik_obj_exact$get_llik_bounds()
...
```{r}
# Plotting samples from log-likelihood distribution.

# Untruncated.
em_llik$plot_samp_1d(test_info$input, em_pred_list=em_pred_llik, N_samp=20L, 
                     true_llik=test_info$llik, include_bounds=TRUE, 
                     adjustment="none", use_cov=TRUE)
# Truncated
# TODO: running into positive definite issues when running this with `use_cov = TRUE`.
em_llik$plot_samp_1d(test_info$input, em_pred_list=em_pred_llik, N_samp=20L, 
                     true_llik=test_info$llik, include_bounds=TRUE, 
                     adjustment="truncated", use_cov=FALSE)

# Rectified.
em_llik$plot_samp_1d(test_info$input, em_pred_list=em_pred_llik, N_samp=20L, 
                     true_llik=test_info$llik, include_bounds=TRUE, 
                     adjustment="rectified", use_cov=TRUE)
```
...
```{r}
# Truncated and rectified log-likelihood distribution.
plt_llik_dist_llik_trunc <- em_llik$plot_pred_1d(test_info$input,
                                           plot_type="llik", include_interval=TRUE,
                                           interval_method=interval_method, N_std_dev=N_std_dev,
                                           true_llik=test_info$llik, CI_prob=CI_prob,
                                           line_thickness=line_thickness,
...
plt_llik_dist_llik_rect <- em_llik$plot_pred_1d(test_info$input,
                                           plot_type="llik", include_interval=TRUE,
                                           interval_method=interval_method, N_std_dev=N_std_dev,
                                           true_llik=test_info$llik, CI_prob=CI_prob,
                                           line_thickness=line_thickness,
...
plot(plt_llik_dist_llik_trunc)
plot(plt_llik_dist_llik_rect)
```
```{r}
# Truncated and rectified likelihood distribution (truncated/rectified 
# log-normal).

test_trunc <- em_llik$calc_lik_marginal_approx(em_pred_list=em_pred_llik, 
                                               log_scale=FALSE, 
                                               adjustment="truncated")
test_rect <- em_llik$calc_lik_marginal_approx(em_pred_list=em_pred_llik, 
                                              log_scale=FALSE, 
                                              adjustment="rectified")

plot(drop(test_info$input), exp(test_info$llik), col="black", type="l")
lines(drop(test_info$input), test_trunc, type="l", col="blue")
lines(drop(test_info$input), test_rect, col="green")
lines(drop(test_info$input), exp(em_pred_llik$mean), col="red")
...
z <- drop((llik_bounds[2] - em_pred_llik$mean) / sqrt(em_pred_llik$var))
lik_pred_trunc$mean <- exp(ln_list$log_mean + 
                             log(pnorm(z-sqrt(em_pred_llik$var))) - 
                             log(pnorm(z)))
lik_pred_trunc$prob_bound_satisfied <- pnorm((llik_bounds[2] - em_pred_llik$mean)/sqrt(em_pred_llik$var))

lik_pred_trunc$mean_rect <- exp(llik_bounds[2])*(1-lik_pred_trunc$prob_bound_satisfied) + 
  lik_pred_trunc$prob_bound_satisfied*lik_pred_trunc$mean
...
plot_pred_1d_helper(drop(test_info$input), pred_mean=lik_pred_trunc$mean,
                    ...
                    design_color=design_color, design_pt_size=design_pt_size) + 
  geom_hline(yintercept=exp(llik_bounds[2]), color="green", linetype="dashed")

# Rectified approximation.
plot_pred_1d_helper(drop(test_info$input), pred_mean=lik_pred_trunc$mean_rect,
                    y_new=exp(test_info$llik), 
                    X_design=drop(design_info$input), 
                    y_design=exp(design_info$llik),
                    design_color=design_color, design_pt_size=design_pt_size) + 
...
```{r}
# Considering multivariate truncation, using smaller number of test points to start.
n_grid_trunc <- 25
trunc_test_info <- get_init_design_list(inv_prob, "tensor_product_grid", n_grid_trunc)
trunc_em_pred_llik <- em_llik$predict_emulator(trunc_test_info$input, return_cov=TRUE)

trunc_pred <- tmvtnorm::mtmvnorm(drop(trunc_em_pred_llik$mean), trunc_em_pred_llik$cov[,,1], 
                                 upper=rep(llik_bounds[2], n_grid_trunc))

trunc_em_pred_llik$mean <- matrix(trunc_pred$tmean, ncol=1)
trunc_em_pred_llik$var <- matrix(diag(trunc_pred$tvar), ncol=1)
trunc_em_pred_llik$cov[,,1] <- trunc_pred$tvar

plt_gp_dist_llik <- em_llik$emulator_model$plot_pred_1d(trunc_test_info$input, include_interval=TRUE,
                                                        interval_method=interval_method, 
                                                        N_std_dev=N_std_dev,
                                                        pred_list=trunc_em_pred_llik, ylab="L(u)",
                                                        Y_new=matrix(trunc_test_info$llik, ncol=1),
                                                        line_thickness=line_thickness,
                                                        design_color=design_color, 
                                                        design_pt_size=design_pt_size, 
                                                        xlab="u", plot_title="GP predictions [llik]")[[1]]

# This isn't right; need to use formula for truncated lognormal mean.

# plt_lik_dist_llik <- em_llik$plot_pred_1d(trunc_test_info$input, em_pred_list=trunc_em_pred_llik,
#                                           plot_type="lik", include_interval=FALSE, 
#                                           true_llik=trunc_test_info$llik, CI_prob=CI_prob,
#                                           line_thickness=line_thickness, 
#                                           log_scale=FALSE, design_color=design_color, 
#                                           design_pt_size=design_pt_size, 
#                                           plot_title="likelihood predictions [llik]")
...
```{r}
# Multivariate truncation, using "virtual points". 
n_grid_virtual <- 10
inputs_virtual <- get_batch_design("tensor_product_grid", N_batch=n_grid_virtual,
                                   prior_params=par_prior)
inputs_comb <- rbind(test_info$input, inputs_virtual)
n_comb <- nrow(inputs_comb)
em_pred_llik_comb <- em_llik$predict_emulator(inputs_comb, return_cov=TRUE)
bound_upper <- c(rep(Inf, N_grid), rep(llik_bounds[2], n_grid_virtual))
eps <- sqrt(.Machine$double.eps)
cov_comb <- em_pred_llik_comb$cov[,,1] + diag(eps, nrow=n_comb)

trunc_pred_virtual <- tmvtnorm::mtmvnorm(drop(em_pred_llik_comb$mean), 
                                         cov_comb, 
                                         upper=bound_upper)
...
# Gaussian predictive distribution at virtual points.
pred_list_virtual <- em_llik$predict_emulator(inputs_virtual, return_mean=TRUE,
                                              return_cov=TRUE)
pred_list_virtual$mean <- drop(pred_list_virtual$mean)
pred_list_virtual$cov <- pred_list_virtual$cov[,,1]


# Truncated Gaussian predictive distribution at virtual points.
upper_bound <- rep(llik_bounds[2], n_grid_virtual)
moments_trunc_virtual <- tmvtnorm::mtmvnorm(pred_list_virtual$mean, pred_list_virtual$cov,
                                            upper=upper_bound, doComputeVariance=TRUE)

# Gaussian predictive distribution at test points, and cross covariance with virtual points.
pred_list_grid <- em_llik$predict_emulator(test_info$input, return_var=FALSE)
...
post_approx_llik <- em_llik$calc_lik_approx(approx_type=c("mean","marginal"),
...
post_approx_llik <- sapply(1:ncol(post_approx_llik), 
                           function(j) normalize_lik(post_approx_llik[,j], norm_cst_llik[j]))
...
post_approx_fwd <- em_fwd$calc_lik_approx(approx_type=c("mean","marginal"),
                                          input=test_info$input,
                                          return_type="matrix", log_scale=FALSE)
...
```{r}
#
# MCWMH approaches: Log-likelihood emulator.
#

common_settings <- list(mcmc_func_name="mcmc_noisy_llik", n_itr=N_mcmc,
                        try_parallel=FALSE, n_chain=4L, itr_start=round(N_mcmc/2),
...
mcmc_settings_llik <- list(
  c(list(test_label="mcwmh-joint-rect-llik", mode="mcwmh", use_joint=TRUE, adjustment="rectified"),
    common_settings),
  c(list(test_label="mcwmh-joint-trunc-llik", mode="mcwmh", use_joint=TRUE, adjustment="truncated"),
    common_settings),
  c(list(test_label="mcwmh-joint-llik", mode="mcwmh", use_joint=TRUE, adjustment="none"),
    common_settings),
  c(list(test_label="mcwmh-ind-rect-llik", mode="mcwmh", use_joint=FALSE, adjustment="rectified"),
    common_settings),
  c(list(test_label="mcwmh-ind-trunc-llik", mode="mcwmh", use_joint=FALSE, adjustment="truncated"),
    common_settings),
  c(list(test_label="mcwmh-ind-llik", mode="mcwmh", use_joint=FALSE, adjustment="none"),
    common_settings),
...
names(mcmc_settings_llik) <- sapply(mcmc_settings_llik, function(x) x$test_label)

```

```{r}
noisy_mcmc_output_llik <- run_mcmc_comparison(em_llik, par_prior, mcmc_settings_llik, return=TRUE)
...
# MCWMH-Ind. 
mcwmh_ind_fwd <- mcmc_noisy_llik(em_fwd, par_prior, n_itr=N_mcmc, 
                                 mode="mcwmh", use_joint=FALSE, adjustment="none")

# MCWMH-Joint. 
mcwmh_joint_fwd <- mcmc_noisy_llik(em_fwd, par_prior, n_itr=N_mcmc, 
                                   mode="mcwmh", use_joint=TRUE, adjustment="none")
