...
samp_list <- mcmc_gp_noisy(llik_sum_emulator, theta_prior_params, par_init=linear_Gaussian_info$true_posterior$mean[,1], 
                           ...
                           cov_prop=linear_Gaussian_info$true_posterior$Cov, SSR_sample_adjustment="truncated")

mcmc_samp_dt <- format_mcmc_output(samp_list["par"], test_label="mcmc_gp_noisy_cov_trunc")
```

```{r}
#
# Run noisy MCMC: Rectified. 
#
# TODO: add option to `format_mcmc_output` that allows selected names from the `samp_list`. 
