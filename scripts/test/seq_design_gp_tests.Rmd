...
plt <- plot_heatmap(test_info$inputs, exp(test_info$outputs), raster=TRUE, log_transform=TRUE)
plot(plt)
...
plt1 <- plot_heatmap(test_info$inputs, test_info$outputs, raster=TRUE) + 
          geom_contour(mapping=aes(x=x1, y=x2, z=y), bins=20)
plt2 <- plot_heatmap(test_info$inputs, exp(test_info$outputs), raster=TRUE, transformation="log") + 
          geom_contour(mapping=aes(x=x1, y=x2, z=log(y)), bins=20)
plt3 <- plot_heatmap(test_info$inputs, test_info$outputs, raster=TRUE, transformation="log") + 
          geom_contour(mapping=aes(x=x1, y=x2, z=exp(y)), bins=20) + 
...
df <- data.frame(x1=test_info$inputs[,1], x2=test_info$inputs[,2], y=test_info$outputs[,1])
test <- ggplot() + 
        geom_contour_filled(aes(x=x1, y=x2, z=exp(y)), data=df) + 
        scale_fill_continuous(trans="log")
plot(test)


df_mvn_samp <- data.frame(x1=rnorm(10000), x2=rnorm(10000))
tester <- ggplot(df_mvn_samp) + 
          stat_density2d(aes(x=x1, y=x2), contour=TRUE, n=100)
plot(tester)

test2 <- MASS::kde2d(df_mvn_samp$x1, df_mvn_samp$x2, n=100)
df_test2 <- expand.grid(test2$x, test2$y) 
colnames(df_test2) <- c("x", "y")
df_test2$z <- melt(test2$z)$value

tester2 <- ggplot(df_test2) + geom_contour(aes(x=x, y=y, z=z))
plot(tester2)

tester2_log <- ggplot(df_test2) + geom_contour(aes(x=x, y=y, z=log(z)))
plot(tester2_log)
```
