...
mcmc_list <- run_mcmc_multichain("mcmc_noisy_llik", llik_exact, n_chain=4L,  
                                 par_prior=par_prior, n_itr=n_mcmc, try_parallel=FALSE)
...
```{r}
mcmc_list <- mcmc_bt_wrapper(llik_exact, par_prior, n_itr=50000L, sampler="DEzs")
samp_dt <- format_mcmc_output(mcmc_list$samp, test_label="bt_DEzs")
...
```{r}
dens2d_plots <- get_2d_density_plots(samp_dt, itr_start=20000L)
for(plt in unlist(dens2d_plots, recursive=FALSE)) plot(plt)
```
