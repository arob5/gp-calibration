...
The two major use cases here are:  

1. Computing deterministic approximations of the likelihood; this is handled 
by the method `calc_lik_approx()`.  
2. Computing error metrics to validate the adequacy of the log-likelihood 
surrogate; this is handled by the method `calc_err_metric()`.  

Note that some likelihood approximations and error metrics will only be 
...
### Error Metrics
The `calc_err_metric()` generically allows the computation of functions of 
the likelihood surrogate that are functions of both the surrogate distribution 
and some baseline external data.
