...
recall that 
\begin{align*}
\logUnPostMeanPred[\designIdx, \batchMat](\bpar) &= \logUnPostMean(\bpar) + \logUnPostKer(\bpar, \designMat[\designIdx, \batchMat]) \KerMat[\designIdx, \batchMat]^{-1} \left(\tilde{\boldsymbol{\postDens}}_{\designIdx, \batchMat} - \logUnPostMean(\bpar) \oneVec \right).
\end{align*}
Thus, 
\begin{align}
...
\E\left[\Exp{\logUnPostKer(\bpar, \designMat[\designIdx, \batchMat]) \KerMat[\designIdx, \batchMat]^{-1} \tilde{\boldsymbol{\postDens}}_{\designIdx, \batchMat}}\right]
\end{align}
Note that the only dependence on $\tilde{\boldsymbol{\postDens}}$ stems from the lower $\Nbatch \times 1$ block of the $(\designIdx + \Nbatch) \times 1$ vector 
\begin{align*}
\tilde{\boldsymbol{\postDens}}_{\designIdx, \batchMat}
&= \begin{pmatrix} \postDens(\designMat)^\top & \postDens(\batchMat)^\top  \end{pmatrix}^\top.
\end{align*}
