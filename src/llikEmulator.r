...
#                    behavior.
# use_fixed_lik_par: logical, if TRUE then `lik_par` must be supplied when an  
#                    object is instantiated, and not changed thereafter.
#                      
...
llikEmulator$methods(
  ...
  initialize = function(llik_label, input_names, lik_description, 
    ...
    if(use_fixed_lik_par && is.null(lik_par)) {
      stop("Fixed `lik_par` not passed but `use_fixed_lik_par` is TRUE.")
    }
  }, 
  ...
  assemble_llik = function(em_val, lik_par_val=NULL, 
    ...
    # numeric vector of length equal to the number of rows in `input`. 
    .NotYetImplemented()
  },
  
  sample = function(input, lik_par_val=NULL, N_samp=1, 
                    conditional=default_conditional,
                    normalize=default_normalize, ...) {
    .NotYetImplemented()
  },
...
llikEmulatorGP$methods(
  ...
  initialize = function(llik_lbl, gp_model, default_conditional, 
    ...
    # Argument checking.
    assert_that(use_fixed_lik_par, 
                msg=paste0("llikEmulatorGP does not yet",
                           "support emulation as a function of `lik_par`."))
  ...
  predict_lik = function(input, lik_par_val=NULL, em_pred_list=NULL, 
    ...
    llik_pred <- .self$predict(input=input, lik_par_val=lik_par_val, 
                               em_pred_list=em_pred_list, return_mean=TRUE, 
  ...
  calc_quantiles = function(p, input=NULL, lik_par_val=NULL, em_pred_list=NULL,
    ...
    # only `llik_pred_list` is required, regardless of whether `target` is
    # "llik" or "lik". 
    ...
    if(target == "llik") {
      q <- qnorm(p, drop(llik_pred_list$mean), sqrt(drop(llik_pred_list$var)),
                 lower.tail=lower_tail)
    } else { 
      q <- qlnorm(p, drop(llik_pred_list$mean), sqrt(drop(llik_pred_list$var)), 
                  lower.tail=lower_tail)
    }
...
llikEmulatorExactGaussDiag$methods(
  ...
  get_lik_par = function(lik_par_val=NULL, ...) {
    ...
    if((length(lik_par_val)==1) && (.self$N_output > 1)) {
      return(rep(lik_par_val, N_output))
    }
    
    assert_that(length(lik_par_val)==N_output, 
  ...
  predict = function(input, lik_par_val=NULL, return_mean=TRUE, return_var=TRUE, 
    ...
    return_list <- list()
    if(return_mean) {
      return_list$mean <- assemble_llik(get_input(input), lik_par_val, 
                                        conditional, normalize)
    }
