...
# stan.fit <- model$sample(data = stan.list, iter_warmup = 0, iter_sampling = 1, 
#                          seed = 494838, fixed_param = TRUE)
# stan.output <- stan.fit$draws()
...
# Map kernel parameters to Stan parameterization
gp.stan.params <- create.gp.params.list(gp.hetGP, "hetGP")
gp.obj <- create.gp.obj(gp.hetGP, "hetGP", X, y)
...
# Test the covariance matrix generated by the design points
C.hetGP <- cov_gen(X, theta = gp.hetGP$theta, type = "Gaussian")
K.hetGP <- gp.hetGP$nu_hat * (C.hetGP + diag(gp.hetGP$g, nrow = N))
K.stan <- stan.output$K_out[1,,]
print(paste0("Cov matrices K(X) equal: ", all.equal(K.hetGP, K.stan)))

# Test the covariance matrix generated by the test/prediction points
C.pred.hetGP <- cov_gen(X.pred, theta = gp.hetGP$theta, type = "Gaussian")
K.pred.hetGP <- gp.hetGP$nu_hat * (C.pred.hetGP + diag(gp.hetGP$g, nrow = N.pred))
K.pred.stan <- stan.output$K_pred[1,,]
print(paste0("Cov matrices K(X_pred) equal: ", all.equal(K.pred.hetGP, K.pred.stan)))

# Test the covariances between the design and prediction points
C.cross.hetGP <- cov_gen(X, X.pred, theta = gp.hetGP$theta, type = "Gaussian")
K.cross.hetGP <- gp.hetGP$nu_hat * C.cross.hetGP
K.cross.stan <- stan.output$K_cross[1,,]
...
print(paste0("Max absolute error in cov matrix predictions (adjusting for jitter): ", 
             max(abs(pred.cov.hetGP - pred.cov.with.jitter))))
