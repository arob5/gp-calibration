...
```{r}
# Samples from true posterior to estimate validation metrics. These are used to evaluate all rounds. These are used to compute
# both sample-based metrics and used to estimate weighted integrals for emulator-based metrics. 

# TODO: create two different sets of samples (one for sample-based metrics and one for emulator-based metrics). Use MCMC thinning to do 
#       so and thin using different sequences to reduce correlation between the two. The set 

# TODO: add relative RMSE metric. 
