...
test <- compute_mcmc_comparison_metric(samp_dt = mcmc_samp_dt, burn_in_start = burn_ins, 
                                       test_labels_1 = "exact", test_labels_2 = round0_labels[1], 
                                       param_types = c("theta", "sig_eps"), 
                                       metrics = c("mean", "cov"))



samp_dt_subset <- select_mcmc_samp(mcmc_samp_dt, burn_in_start = burn_ins, test_labels = c("exact", round0_labels[1]), 
                                   param_types = "theta", param_names = param_names)
means <- samp_dt_subset[, .(samp_mean = mean(sample)), by = .(test_label, param_type, param_name)]


# If there is only one param_name within param_type_aggregate value then no aggregation can be done and only non-aggregated 
