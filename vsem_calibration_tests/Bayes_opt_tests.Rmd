...
new_point_scaled <- acquisition_opt_one_step(emulator_info_list = emulator_info_list, acquisition_settings = acquisition_settings, 
                                             computer_model_data = computer_model_data, sig2_eps = sig_eps_estimates)
new_point <- scale_input_data(new_point_scaled, input_bounds = design_info$input_bounds, inverse = TRUE)
print(new_point)
...
- Summary: 
    ...
    (ii.) Metrics on prob dists, computed from samples: keep researching; MMD seems a good option, among others 
