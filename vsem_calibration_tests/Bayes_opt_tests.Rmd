...
time_start <- proc.time()
mcmc_approx_round1 <- mcmc_calibrate_product_lik(computer_model_data = computer_model_data, 
                                                 theta_prior_params = theta_prior_params,
...
samp_approx_round1_df <- as.data.frame(mcmc_approx_round1$samp)
colnames(samp_approx_round1_df) <- paste("round1", colnames(samp_approx_round1_df), sep = "_")
mcmc_samp_df <- cbind(mcmc_samp_df, samp_approx_round1_df)
...
mcmc_round1_trace_plts <- get_trace_plots(mcmc_samp_df, burn_in_start = burn_ins, param_types = "theta", test_labels = "round1")
grid.arrange(arrangeGrob(grobs = mcmc_round1_trace_plts, nrow = 1))
```

```{r}
mcmc_round1_trace_plts_sig_eps <- get_trace_plots(mcmc_samp_df, burn_in_start = burn_in_exact, param_types = "sig_eps", test_labels = "round1")
...
acq_grid_EIVAR_approxpost_lpost <- acquisition_EIVAR_lpost(theta_vals = prior_grid_info$inputs_scaled, 
                                                           ...
                                                           theta_prior_params = theta_prior_params, 
                                                           sig2_eps = sig2_eps_estimates, 
                                                           theta_grid_integrate = scale_input_data(select_mcmc_samp(mcmc_samp_df = mcmc_samp_df, 
                                                                                                                    burn_in_start = burn_ins, 
                                                                                                                    param_types = "theta", 
                                                                                                                    test_labels = "round1"), 
                                                                                                   input_bounds = design_info$input_bounds))
...
acq_EI_lpost_MC_plt <- get_2d_heatmap_plot(X = prior_grid_info$inputs, 
...
acq_VAR_lpost_plt <- get_2d_heatmap_plot(X = prior_grid_info$inputs, 
