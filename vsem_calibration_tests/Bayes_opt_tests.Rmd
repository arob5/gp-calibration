...
# TODO: need to monitor estimated auto-correlation here. 
...
samp_exact_theta_metrics <- samp_exact_theta[seq(1, nrow(samp_exact_theta), 10),]

# TODO: need to deal with the fact that the scaling is different for each design. 
...
SSR_outputs <- get_computer_model_SSR(computer_model_data, theta_vals = samp_exact_theta_metrics, na.rm = TRUE)

# Compute emulator-based and sample-based metrics for round 0. 
test <- get_lpost_emulator_metric_comparison(lpost_emulator_list = lpost_emulators, 
                                             lpost_validation_inputs = samp_exact_theta_metrics, 
                                             metrics = c("crps", "rmse"), 
                                             SSR_outputs = SSR_outputs)

predict_lpost_emulator(inputs_new_scaled)
test <- compute_gp_metrics(gp_mean_pred, output_true, metrics, gp_var_pred = NULL, transformation_method = NA_character_, 
                           gp_cov_pred = NULL, gp_L_pred = NULL)


# TODO: issue - making computer_model_data arg optional in lpost function is causing an issue cause currently the function grabs `n_obs` from 
#       computer model data. Need to decide how to handle this. 
...
# Emulator metrics. 
test <- compute_lpost_emulator_metrics(lpost_mean_pred, lpost_true, metrics, lpost_var_pred = NULL, transformation_method = NA_character_, 
                                       lpost_cov_pred = NULL, lpost_L_pred = NULL)




suppressWarnings(emulator_metrics <- get_emulator_comparison_metrics_validation_list(emulator_info_list = emulator_info_list, 
