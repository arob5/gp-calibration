...
# TODO: TEMP, Emulator that centers but does not scale response. Temporarily keep this until I clean up transforamtion code. 
...
upper_CI <- pred_grid_init_design$mean + 2*sqrt(pred_grid_init_design$var)
lower_CI <- pred_grid_init_design$mean - 2*sqrt(pred_grid_init_design$var)
...
curr_post_log_var <- convert_to_post_emulator_log_moments(lpost_emulator_no_scale, pred_grid_init_design$mean, pred_grid_init_design$var, "var")$var
...
curr_post_log_var <- convert_to_post_emulator_log_moments(lpost_emulator_no_scale, pred_grid_init_design$mean, pred_grid_init_design$var, "log_var")$log_var
post_cond_log_var_1 <- convert_to_post_emulator_log_moments(lpost_emulator_cond_1, pred_cond_1$mean, pred_cond_1$var, "log_var")$log_var
post_cond_log_var_2 <- convert_to_post_emulator_log_moments(lpost_emulator_cond_2, pred_cond_2$mean, pred_cond_2$var, "log_var")$log_var
...
u_tilde <- prior_grid_info$inputs_scaled[600,,drop=FALSE] # 999

# Unconditioned log variance. 
un_lpost_pred <- predict_lpost_emulator(u, lpost_emulator_no_scale, uncenter = FALSE)
un_log_var <- convert_to_post_emulator_log_moments(lpost_emulator_no_scale, un_lpost_pred$mean, un_lpost_pred$var)$log_var
...
log_pi_tilde_pred <- convert_to_post_emulator_log_moments(lpost_emulator_no_scale, lpost_tilde_pred$mean, lpost_tilde_pred$var)
N_MC_test <- 10000
...
np1 <- length(lpost_emulator_no_scale_cond$outputs_lpost)
...
for(j in seq_len(N_MC_test)) {
  ...
  # Update response. 
  lpost_emulator_no_scale_cond$outputs_lpost[np1] <- lpost_tilde_samp[j]
  
  # Compute conditioned variance. 
  results <- predict_lpost_emulator(u, lpost_emulator_no_scale_cond, uncenter = FALSE)
...
cond_log_var_MC_samp <- convert_to_post_emulator_log_moments(lpost_emulator_no_scale_cond, cond_lpost_mean_samp, cond_lpost_var_samp)
...
pi_tilde_samp <- rlnorm(n = 1000, pred_cond_2$mean, sqrt(pred_cond_2$var))
MC_pred <- vector(mode = "numeric", length = nrow(prior_grid_info$inputs_scaled))

for(i in seq_along(MC_pred)) {
