...
# robustness to such bad designs. 
...
samp_round0_theta <- select_mcmc_samp(mcmc_samp_dt, burn_in_start = burn_ins, test_labels = round0_labels[1], param_types = "theta")[, .(param_name, sample)]
...
lpost_emulator_new_batch <- batch_acquisition_opt_one_step(lpost_emulators[[1]], acquisition_settings)
...
new_batch_sel <- c(rep(FALSE, nrow(lpost_emulator_new_batch$inputs_lpost$inputs) - acquisition_settings$batch_size), 
                   rep(TRUE, acquisition_settings$batch_size))


SSR_vals <- get_computer_model_SSR(computer_model_data = computer_model_data, 
                                   theta_vals = samp_round0_theta, na.rm = TRUE)
lprior_vals <- calc_lprior_theta(samp_round0_theta, theta_prior_params)

lpost_vals_round0 <- calc_lpost_theta_product_lik(computer_model_data = computer_model_data, 
                                                  theta_vals = samp_round0_theta, 
                                                  SSR = SSR_vals, 
                                                  vars_obs = lpost_emulator_new_batch$sig2_eps, 
...
pred <- predict_lpost_emulator(lpost_emulator = lpost_emulators[[1]], 
                               inputs_new_scaled = scale_input_data(samp_round0_theta, input_bounds = lpost_emulators[[1]]$emulator_info_list$input_bounds), 
                               inputs_new_unscaled = samp_round0_theta, include_nugget = TRUE)

plt_mean <- get_2d_heatmap_plot(X = samp_round0_theta, y = pred$mean, param_names = computer_model_data$pars_cal_names, 
                                samples_kde = samp_exact_theta, raster = FALSE, 
                                samples_points = lpost_emulator_new_batch$inputs_lpost$inputs[!new_batch_sel,,drop=FALSE], 
                                point_coords = computer_model_data$theta_true, bigger_is_better = TRUE)
plt_var <- get_2d_heatmap_plot(X = samp_round0_theta, y = drop(pred$var), param_names = computer_model_data$pars_cal_names, 
                               samples_kde = samp_exact_theta, raster = FALSE, 
                               samples_points = lpost_emulator_new_batch$inputs_lpost$inputs[!new_batch_sel,,drop=FALSE], 
