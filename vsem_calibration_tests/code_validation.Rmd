...
```
```{r}
# Add truncated proposal samples to the main list for convenience. 
calibration_results_list$mcmc_approx_list[["trunc_proposal"]] <- list(samp_mcmc = mcmc_trunc_prop_samp, mcmc_runtime = mcmc_trunc_prop_runtime)
```


## Comparing marginal distributions. 
```{r}
# TODO: add mcmc_trunc_prop_samp to list `calibration_results_list$mcmc_approx_list` and then write function called something like 
# `compare_calibration_results_marginal_samples()` to produce these plots. 


# TODO: plotting function should accept arguments "test_labels" and "param_names", and will automatically form the correct column names 
# by appending the two. Alternatively, can have additional argument called "col_names" that overrides the the column selection and allows 
# the user to directly select the columns to plot by name. 
...
# compare_calibration_results_marginal_samples <- function() {
#   
# }
