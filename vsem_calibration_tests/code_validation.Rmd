...
```{r}
# Questions:
#    - Why is projection onto first dimension out of control but second dimension looks reasonable? 
#    - TODO: need to think about confidence intervals for two-dimensional plots. 
#    - What is up with jagged true function in second dimension? 
...
for(j in seq_along(gp_pred_list)) {
  ...
  gp_plot <- plot_gp_fit_2d(test_data$inputs, test_data$outputs[,j], train_data$inputs, 
                            ...
                            gp_pred_list[[j]]$var, xlab = "Parameter", ylab = "SSR",
                            main_title = computer_model_data$output_vars[j], CI_prob = 0.9, 
...
samp_GP_approx <- mcmc_calibrate_ind_GP(computer_model_data, theta_prior_params, emulator_info, learn_Sig_eps = FALSE,
                                        Sig_eps_init = computer_model_data$Sig_eps, Sig_eps_prior_params = Sig_eps_prior_params, N_mcmc = 50000)
```
...
plot(samp_GP_approx$Sig_eps[burn_in:N_mcmc,1], type="l")
```
