...
samp <- mcmc_calibrate(computer_model_data, theta_prior_params, diag_cov = TRUE, learn_Sig_eps = FALSE, 
                       ig_eps_init = diag(1.0), N_mcmc = N_mcmc, adapt_cov_method = "AM", adapt_scale_method = "MH_ratio")
```
...
```{r}
samp_baseline <- mcmc_calibrate(computer_model_data, theta_prior_params, diag_cov = TRUE, learn_Sig_eps = FALSE, 
...
for(j in seq_along(gp_pred_list)) {
  ...
  gp_plot <- plot_gp_fit_2d(test_data$inputs, test_data$outputs[,j], train_data$inputs, 
                            ...
                            main_title = "GP Predictive Density; Contours of True Posterior",
...
```{r}
# Plot 2D response surface: true loss, GP predicted loss (mean), GP predictive variance, true posterior

plt <- plot_2d_response_surface(test_data$inputs, )
print(plt)
...
samp_GP_approx <- mcmc_calibrate_ind_GP(computer_model_data, theta_prior_params, emulator_info, learn_Sig_eps = FALSE,
                                        Sig_eps_init = computer_model_data$Sig_eps, N_mcmc = N_mcmc)
```

#### Approximate MCMC
```{r}
burn_in <- 25000
plot(samp_GP_approx$theta[burn_in:N_mcmc,1], type="l")
plot(samp_GP_approx$theta[burn_in:N_mcmc,2], type="l")
```
...
# Histograms.
for(j in seq(1,d)) {
  hist_plot <- get_hist_plot(list(samp_baseline$theta[burn_in:N_mcmc,], samp_GP_approx$theta[burn_in:N_mcmc,]), col_sel = j, bins = 80, xlab = computer_model_data$pars_cal_name[j],
...
```{r}
samp_baseline_2 <- mcmc_calibrate(computer_model_data, theta_prior_params, diag_cov = TRUE, learn_Sig_eps = TRUE, 
                                Sig_eps_prior_params = Sig_eps_prior_params, N_mcmc = N_mcmc, adapt_cov_method = "AM", adapt_scale_method = "MH_ratio")
```
...
# Histograms: theta. 
for(j in seq(1,d)) {
  hist_plot <- get_hist_plot(list(samp_baseline_2$theta[burn_in:N_mcmc,], samp_GP_approx_2$theta[burn_in:N_mcmc,]), col_sel = j, 
...
# Histogram: sig2_eps. 
hist_plot_sig2_eps <- get_hist_plot(list(samp_baseline_2$Sig_eps[burn_in:N_mcmc,,drop=FALSE], samp_GP_approx_2$Sig_eps[burn_in:N_mcmc,,drop=FALSE]), 
