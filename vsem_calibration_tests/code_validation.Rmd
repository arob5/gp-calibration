...
linear_Gaussian_data <- generate_linear_Gaussian_test_data(random_seed = 1, N_obs = N_obs, D = d, sig2_eps = 1.0, 
...
colnames(data_obs) <- "y"
computer_model_data <- list(f = function(theta) {matrix(theta, ncol=1)}, 
                            data_obs = data_obs, 
                            n_obs = 2, 
                            output_vars = "y", 
...
```{r}
# Verify likelihood computation is correct. Distribution should be N_2(theta|0, sig2_eps*I)
sig2_eps <- matrix(1.0, nrow=1)
L_sig2_eps <- t(chol(sig2_eps))
theta <- c(1, 2)

model_errs <- get_computer_model_errs(theta, computer_model_data)
llik_curr <- llik_Gaussian_err(model_errs, L = L_sig2_eps) 
llik_baseline <- dnorm(theta[1], sd=sqrt(sig2_eps), log=TRUE) + dnorm(theta[2], sd=sqrt(sig2_eps), log=TRUE)
...
```{r}
# TODO: test non-adaptive with known posterior covariance, scaled using optimal scaling for Gaussians. 
N_mcmc <- 50000
...
```{r}
hist(samp$theta[burn_in:N_mcmc,1], 50)
hist(samp$theta[burn_in:N_mcmc,2], 50)
```

```{r}
stats::cov(samp$theta[burn_in:N_mcmc,])
```
