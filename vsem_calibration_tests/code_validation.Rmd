...
# Plot GPs. 
for(j in seq_along(gp_pred_list)) {
  for(k in seq_along(computer_model_data$theta_true)) {
    gp_plot <- plot_gp_fit_1d(test_data$inputs[,k,drop=FALSE], test_data$outputs[,j], train_data$inputs[,k,drop=FALSE], 
