...
samp_exact_theta <- samp_exact_df[burn_in_exact:N_mcmc_exact, c("exact_theta_LAR", "exact_theta_KEXT")]
colnames(samp_exact_theta) <- c("LAR", "KEXT")

for(output_variable in computer_model_data$output_vars) {
  plt <- get_2d_heatmap_plot(X = theta_prior_design$inputs, 
                             y = theta_prior_design$outputs[, output_variable], 
                             param_names = computer_model_data$pars_cal_names,
                             samples = samp_exact_theta, 
                             raster = TRUE, main_title = output_variable, 
                             point_coords = computer_model_data$theta_true)
  plot(plt)
}
```
...
```{r}
SSR_post <- get_computer_model_SSR(computer_model_data, theta_vals = as.matrix(samp_exact_theta))
```

```{r}
for(output_variable in computer_model_data$output_vars) {
  plt <- get_2d_heatmap_plot(X = samp_exact_theta, 
                             y = SSR_post[, output_variable], 
                             param_names = computer_model_data$pars_cal_names,
                             samples = samp_exact_theta, 
                             raster = FALSE, main_title = output_variable, 
                             point_coords = computer_model_data$theta_true)
  plot(plt)
}
```
...
design_settings <- data.frame(N_design = c(15, 20, 25), 
                              design_method = c("LHS", "LHS", "LHS"), 
                              scale_inputs = TRUE, 
                              normalize_response = TRUE)

design_list <- get_design_list(design_settings, computer_model_data, theta_prior_params, reps = 10)
...
validation_list_post <- vector(mode = "list", length = N_test_sets)
for(j in seq_along(validation_list_post)) {
  idx <- sample(seq_len(nrow(samp_exact_theta)), size = N_test_points, replace = FALSE)
  validation_list_post[[j]]$inputs <- samp_exact_theta[idx,]
  validation_list_post[[j]]$outputs <- SSR_post[idx,]
}
```
