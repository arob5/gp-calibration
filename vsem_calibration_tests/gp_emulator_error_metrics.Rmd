...
samp_approx <- calibration_comparison_list$mcmc_approx_list[[1]]$samp_mcmc
```
...
plot(samp_approx$theta[burn_in_approx:N_mcmc_approx,1], type="l", main = "Approx MCMC", ylab = colnames(samp_approx$theta)[1])
plot(samp_approx$theta[burn_in_approx:N_mcmc_approx,2], type="l", main = "Approx MCMC", ylab = colnames(samp_approx$theta)[2])
plot(samp_approx$sig_eps[burn_in_approx:N_mcmc_approx,1], type="l", main = "Approx MCMC", ylab = paste0("Variance: ", colnames(samp_approx$sig_eps)[1]))
plot(samp_approx$sig_eps[burn_in_approx:N_mcmc_approx,2], type="l", main = "Approx MCMC", ylab = paste0("Variance: ", colnames(samp_approx$sig_eps)[2]))
```
...
for(j in 1:ncol(samp_baseline$theta)) {
  hist_plot <- get_hist_plot(list(samp_baseline$theta[burn_in_baseline:N_mcmc_exact,], samp_approx$theta[burn_in_approx:N_mcmc_approx,]), 
...
for(j in 1:ncol(samp_baseline$sig_eps)) {
  hist_plot <- get_hist_plot(list(samp_baseline$sig_eps[burn_in_baseline:N_mcmc_exact,], samp_approx$sig_eps[burn_in_approx:N_mcmc_approx,]), 
...
gp_pred_list <- calibration_comparison_list$mcmc_approx_list[[1]]$gp_pred_list
...
for(j in seq_along(gp_pred_list)) {
  ...
  gp_plot <- plot_gp_fit_2d(X_test = test_data$inputs, 
                            ...
                            y_test = test_data$outputs[,j], 
                            gp_mean_pred = gp_pred_list[[j]]$mean, 
                            gp_var_pred = gp_pred_list[[j]]$var, 
                            ...
                            main_title = paste0(computer_model_data$output_vars[j], 
                                                " GP Predictive Density; Contours of True Posterior"),
...
nlpd_err_list <- estimate_integrated_neg_log_pred_density_err(computer_model_data, 
                                                              ...
                                                              emulator_info = calibration_comparison_list$mcmc_approx_list[[1]]$emulator_info)
```
...
gp_plot <- plot_gp_fit_2d(X_test = samp_baseline$theta[burn_in_baseline:N_mcmc_exact,],
                          ...
                          xlab = colnames(samp_baseline$theta)[1], 
                          ylab = colnames(samp_baseline$theta)[2], 
                          main_title = "SSR GP Predictive Density at Posterior Samples; Contours of True Posterior", 
                          log_predictive_density = -nlpd_err_list$neg_log_pred_density)
