...
$$ \mathcal{E}_{\text{NLPD}}(T^*) = \int \log p_{T^*}(T(\theta)) \pi(\theta) d\theta = \int \sum_{j = 1}^{p} \log_{T_j^*}(T_j(\theta)) \pi(\theta) d\theta$$
with a Monte Carlo estimate
$$ \hat{\mathcal{E}}_{\text{NLPD}}(T^*) = \sum_{i = 1}^{M} \sum_{j = 1}^{p} \log_{T_j^*}(T_j(\theta_i)), \text{ where } \theta_i \sim \pi$$
```{r}
