...
$$
\hat{\pi}(\theta) \sim \mathcal{LN}\left(-\frac{p}{2}\log(2\pi) - \frac{1}{2}\sum_{j = 1}^{p} \log(\sigma_j^2) + \log\pi(\theta) - \frac{1}{2} \sum_{j = 1}^{p} \frac{\mu_j^*(\theta)}{\sigma_j^2}, \frac{1}{4} \sum_{j = 1}^{p} \frac{k_j^*(\theta)}{\sigma_j^4} \right)
$$
...
post_approx_LNP_params_test <- get_gp_approx_posterior_LNP_params(diag(test1_list$Sig_eps), lprior_theta_test, SSR_pred_floor, GP_pred_vars)
...
for(j in seq_along(gp_plots)) {
  gp_plots[[j]] <- plot_gp_fit_1d(theta_grid, SSR_test[,j], train_test_data$X_train, train_test_data$Y_train[,j], gp_pred[[j]]$mean, gp_pred[[j]]$sd2, 
...
for(j in seq_along(gp_plots)) {
  gp_plot <- plot_gp_fit_1d(theta_grid, SSR_test[,j], train_test_data$X_train, train_test_data$Y_train[,j], gp_pred[[j]]$mean, gp_pred[[j]]$sd2, 
...
### Posterior Plots
```{r, echo = FALSE}
post_approx_plot <- plot_gp_fit_1d(theta_grid, lpost_theta_pred, train_test_data$X_train, lpost_theta_train, post_approx_LNP_log_mean_test, 
...
plot(theta_grid, -lpred_pi_test, type = "l", main = "Posterior Loss", xlab = "theta", ylab = "Negative Log Predictive Density of Posterior Approx")
...
# Plot 2: Zoom in
select_subset <- (theta_grid >= 0.4)
plot(theta_grid[select_subset], lpost_theta_test[select_subset], type = "l", col = "red", main = "True vs. Approximate Log Posterior", xlab = "theta", ylab = "Log Posterior")
lines(theta_grid, lpost_theta_pred, col = "blue")
points(train_test_data$X_train, lpost_theta_train, col = "red", pch = 16)

# Plot 3: Zoom in
select_subset <- (theta_grid >= 0.4) & (theta_grid < 0.45)
plot(theta_grid[select_subset], lpost_theta_test[select_subset], type = "l", col = "red", main = "True vs. Approximate Log Posterior", xlab = "theta", ylab = "Log Posterior")
lines(theta_grid, lpost_theta_pred, col = "blue")
points(train_test_data$X_train, lpost_theta_train, col = "red", pch = 16)
```
