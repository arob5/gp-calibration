...
N_design_points <- 10
N_test <- 1000
train_test_data <- get_train_test_data(N_design_points, N_test, theta_prior_params, extrapolate = FALSE, ref_pars = test1_list$ref_pars, 
                                       pars_cal_sel = test1_list$pars_cal_sel, data_obs = test1_list$data_obs,
                                       PAR = test1_list$PAR_data, output_vars = test1_list$output_vars, scale_X = TRUE, 
                                       normalize_Y = TRUE, log_SSR = FALSE, method = "grid", order_1d = TRUE)
```

```{r, include = FALSE}
# TEST
train_test_data_settings <- data.frame(N_train = c(5, 8), 
                                       N_test = c(1000), 
                                       extrapolate = FALSE, 
                                       scale_X = TRUE, 
                                       normalize_Y = TRUE, 
...
emulator_tests <- run_emulator_comparison(vsem_test_case_num = 1, emulator_settings, theta_prior_params, train_test_data_settings)


# test <- run_emulator_test(computer_model_data = test1_list, emulator_settings[1,], theta_prior_params, train_test_data)
...
for(i in seq(1, nrow(emulator_settings))) {
  for(j in seq(1, nrow(train_test_data_settings))) {
    post_approx_plot <- plot_gp_fit_1d(emulator_tests$theta_train_test_data[[j]]$X_test, 
                                       emulator_tests$test_results[[i]][[j]]$ground_truth_quantities$lpost_test, 
                                       emulator_tests$theta_train_test_data[[j]]$X_train,
                                       emulator_tests$test_results[[i]][[j]]$ground_truth_quantities$lpost_train, 
                                       emulator_tests$test_results[[i]][[j]]$post_pred_mean_log_test,
                                       emulator_tests$test_results[[i]][[j]]$post_pred_var_log_test,
                                       main_title = "GP Log Posterior Approximation", xlab = "KEXT",
                                       ylab = "Log Posterior Density", vertical_line = true_param_val, log_scale = FALSE, exponentiate_predictions = FALSE)
    print(post_approx_plot)
  }
}
```
...
# theta_grid <- matrix(seq(min(train_test_data$X_train), max(train_test_data$X_train), length.out = N_test), ncol = 1)
...
model_outputs_list <- lapply(train_test_data$X_test, 
                             function(theta) run_VSEM(theta, test1_list$ref_pars, test1_list$pars_cal_sel, test1_list$PAR_data, test1_list$output_vars))
SSR_test <- calc_SSR(test1_list$data_obs, model_outputs_list, na.rm = TRUE) 
llik_test_per_output <- sapply(seq(1, nrow(SSR_test)), 
                               function(i) llik_Gaussian_SSR(SSR_test[i,], diag(test1_list$Sig_eps), test1_list$n_obs, normalize = TRUE))
...
lprior_theta_test <- sapply(seq(1, nrow(train_test_data$X_test)), function(i) calc_lprior_theta(train_test_data$X_test[i,], theta_prior_params))
...
```{r, include = FALSE}
# Predict with GPs
gp_pred <- predict_independent_GPs(train_test_data$X_test_preprocessed, gp_fits, gp_lib, cov_mat = FALSE, denormalize_predictions = TRUE,
                                   output_stats = train_test_data$output_stats, exponentiate_predictions = FALSE)
gp_pred_train <- predict_independent_GPs(train_test_data$X_train_preprocessed, gp_fits, gp_lib, cov_mat = FALSE, denormalize_predictions = TRUE,
                                         output_stats = train_test_data$output_stats, exponentiate_predictions = FALSE)
```
...
SSR_pred_train <- sapply(gp_pred_train, function(pred) pred$mean)
GP_pred_vars_train <- sapply(gp_pred_train, function(pred) pred$sd2)

# Floor negative predictions at 0
SSR_pred_floor <- SSR_pred
SSR_pred_floor[SSR_pred_floor < 0] <- 0
SSR_pred_train_floor <- SSR_pred_train
SSR_pred_train_floor[SSR_pred_train_floor < 0] <- 0
...
for(j in seq(1, ncol(GP_losses))) {
  GP_losses[,j] <- GP_pointwise_loss(SSR_test[,j], SSR_pred_floor[,j], GP_pred_vars[,j])  
}

# GP approximated posterior
llik_pred_test_per_output <- sapply(seq(1, nrow(SSR_pred)), function(i) llik_Gaussian_SSR(SSR_pred[i,], diag(test1_list$Sig_eps), test1_list$n_obs, normalize = TRUE))
llik_pred_test <- colSums(llik_pred_test_per_output)
lpost_theta_pred_test <- llik_pred_test + lprior_theta_test
```

```{r, echo = FALSE}
# Calculate log predictive density of posterior 
# lpred_pi_test <- gp_approx_posterior_pred_log_density(lpost_theta_test, diag(test1_list$Sig_eps), lprior_theta_test, SSR_pred_floor, GP_pred_vars)
# lpred_pi_train <- gp_approx_posterior_pred_log_density(lpost_theta_train, diag(test1_list$Sig_eps), lprior_theta_train, SSR_pred_train_floor, GP_pred_vars_train)

# Log mean and log variance of predictive distribution
post_approx_LNP_params_test <- get_gp_approx_posterior_LNP_params(diag(test1_list$Sig_eps), lprior_theta_test, SSR_pred_floor, GP_pred_vars, test1_list$n_obs)
post_approx_LNP_log_mean_test <- post_approx_LNP_params_test$mean_log
post_approx_LNP_log_var_test <- post_approx_LNP_params_test$var_log

# Log predictive density of posterior approximation
lpred_pi_test <- gp_approx_posterior_pred_log_density(lpost_theta_test, diag(test1_list$Sig_eps), lprior_theta_test, SSR_pred_floor, GP_pred_vars, test1_list$n_obs, 
                                                      lnorm_log_mean = post_approx_LNP_log_mean_test, lnorm_log_var = post_approx_LNP_log_var_test)
lpred_pi_train <- gp_approx_posterior_pred_log_density(lpost_theta_train, diag(test1_list$Sig_eps), lprior_theta_train, SSR_pred_train_floor, GP_pred_vars_train, test1_list$n_obs)
```
...
### Posterior Plots
```{r, echo = FALSE}
post_approx_plot <- plot_gp_fit_1d(train_test_data$X_test, lpost_theta_test, train_test_data$X_train, lpost_theta_train, post_approx_LNP_log_mean_test, 
                                   post_approx_LNP_log_var_test, main_title = "GP Log Posterior Approximation", xlab = "KEXT",
                                   ylab = "Log Posterior Density", vertical_line = true_param_val, log_scale = FALSE, exponentiate_predictions = FALSE)
print(post_approx_plot)
```
...
```{r, echo = FALSE}
pct_y_keep <- .1
plot_y_scale <- layer_scales(post_approx_plot)$y$range$range
ylim_max <- plot_y_scale[2]
ylim_min <- ylim_max - pct_y_keep * diff(plot_y_scale)
suppressWarnings(print(post_approx_plot + ylim(ylim_min, ylim_max)))
```
...
# Sampling from Posterior to Estimate Error Metrics 
```{r}
N_mcmc <- 100000
post_samples <- mcmc_calibrate(test1_list$ref_pars, test1_list$pars_cal_sel, test1_list$data_obs, test1_list$output_vars, test1_list$PAR_data,
                               theta_prior_params = theta_prior_params, learn_Sig_eps = FALSE, diag_cov = TRUE, 
...
```{r}
# Trace plot
idx_samples_keep <- floor(N_mcmc / 2)
theta_samples <- post_samples$theta[idx_samples_keep:N_mcmc, 1]
plot(theta_samples, type = "l", main = "Trace Plot: KEXT", ylab = "KEXT")
acf(theta_samples, lag = 50, col = "red", main = "Autocorrelation Plot: KEXT")
hist(theta_samples, 100, main = "Histogram: KEXT MCMC Samples", xlab = "KEXT")
```

```{r, echo = FALSE}
# Estimate loss 
lpred_pi_mcmc_samples <- gp_approx_posterior_pred_log_density(lpost_theta_test, diag(test1_list$Sig_eps), lprior_theta_test, SSR_pred_floor, GP_pred_vars, test1_list$n_obs, 
                                                              lnorm_log_mean = post_approx_LNP_log_mean_test, lnorm_log_var = post_approx_LNP_log_var_test)
```
...
# # Plot 1: true vs. approximate log posterior
# plot(train_test_data$X_test, lpost_theta_test, type = "l", col = "red", main = "True vs. Approximate Log Posterior", xlab = "theta", ylab = "Log Posterior")
# lines(train_test_data$X_test, lpost_theta_pred, col = "blue")
# points(train_test_data$X_train, lpost_theta_train, col = "red", pch = 16)
# 
# # Plot 2: Zoom in
# select_subset <- (train_test_data$X_test >= 0.4)
# plot(train_test_data$X_test[select_subset], lpost_theta_test[select_subset], type = "l", col = "red", main = "True vs. Approximate Log Posterior", xlab = "theta", ylab = "Log Posterior")
# lines(train_test_data$X_test, lpost_theta_pred, col = "blue")
# points(train_test_data$X_train, lpost_theta_train, col = "red", pch = 16)
# 
# # Plot 3: Zoom in
# select_subset <- (train_test_data$X_test >= 0.4) & (train_test_data$X_test < 0.45)
# plot(train_test_data$X_test[select_subset], lpost_theta_test[select_subset], type = "l", col = "red", main = "True vs. Approximate Log Posterior", xlab = "theta", ylab = "Log Posterior")
# lines(train_test_data$X_test, lpost_theta_pred, col = "blue")
# points(train_test_data$X_train, lpost_theta_train, col = "red", pch = 16)
```
...
TODO: Need to adapt this to apply to the actually GP fits on SSR, but I believe the same ideas apply there. 
The stationarity assumption appears to be somewhat of an issue here. In the left part of the interval the true function changes rapidly, while on the remaining part of 
the interval it changes very slowly. The GP fit ends up being too wiggly, presumably because it has to choose a single lengthscale to compromise between these two 
regimes; the left portion drives the lengthscale to be smaller, which results in an overly wiggly fit in the remainder of the interval. I don't think this fully captures the issue either though; it seems like the optimal kernel parameters on the lefthand portion would be a very large range and a pretty large lengthscale as well. It seems like this portion could be well-approximated by a function that is pretty linear. The righthand side has 
