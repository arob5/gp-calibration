...
```{r, echo = FALSE}
# Emulator settings
emulator_settings <- data.frame(gp_lib = c("mlegp"), 
                                kernel = "Gaussian", 
...
theta_prior_params[,"param1"] <- 0.4 # theta_prior_params[,"lower"]
theta_prior_params[,"param2"] <- 0.6 # theta_prior_params[,"upper"]
...
train_data <- get_input_output_design(N_train, theta_prior_params, computer_model_data = computer_model_data,  
                                      ...
                                      transformation_method = emulator_settings$transformation_method, design_method = "LHS", order_1d = TRUE) 
test_data <- get_input_output_design(N_test, theta_prior_params, computer_model_data = computer_model_data,  
                                     scale_inputs = TRUE, normalize_response = TRUE, param_ranges = train_data$input_bounds, output_stats = train_data$output_stats,
                                     transformation_method = emulator_settings$transformation_method, design_method = "grid", order_1d = TRUE) 
```
...
gp_pred_list <- predict_independent_GPs(X_pred = test_data$inputs_scaled, gp_obj_list = gp_fits, gp_lib = emulator_settings$gp_lib, 
                                        denormalize_predictions = TRUE, output_stats = train_data$output_stats)
```

### Plot GPs. 
```{r}
for(j in seq_along(gp_pred_list)) {
  gp_plot <- plot_gp_fit_1d(test_data$inputs, test_data$outputs[,j], train_data$inputs, train_data$outputs[,j], gp_pred_list[[j]]$mean, gp_pred_list[[j]]$var,
                            xlab = "Parameter", ylab = "SSR", main_title = computer_model_data$output_vars[j], CI_prob = 0.9, 
...
# Samples from Exact Posterior
```{r}
# TODO: Sig_eps_prior_params does not appear to be defined anywhere here. 
N_mcmc <- 50000
post_samples <- mcmc_calibrate(computer_model_data$ref_pars, computer_model_data$pars_cal_sel, computer_model_data$data_obs, 
                               computer_model_data$output_vars, computer_model_data$PAR_data, computer_model_data$n_obs,
                               theta_prior_params = theta_prior_params, learn_Sig_eps = TRUE, Sig_eps_prior_params = Sig_eps_prior_params,  
                               diag_cov = TRUE, N_mcmc = N_mcmc, adapt_frequency = 1000, adapt_min_scale = 0.1, accept_rate_target = 0.24, 
                               proposal_scale_decay = 0.7, proposal_scale_init = 0.1)

idx_samples_keep <- floor(N_mcmc / 2)
theta_samples <- post_samples$theta[idx_samples_keep:N_mcmc,, drop = FALSE]
```

```{r}
plot(theta_samples, type = "l", main = "Trace Plot", ylab = computer_model_data$pars_cal_names)
acf(theta_samples, lag = 50, col = "red", main = paste0("Autocorrelation Plot: ", computer_model_data$pars_cal_name))
```

# Approximate Calibration with GP Emulators.
```{r}
N_mcmc <- 100000
post_approx_samples <- mcmc_calibrate_ind_GP(computer_model_data, emulator_info, theta_prior_params, 
                                             theta_init = NULL, Sig_eps_init = NULL, learn_Sig_eps = TRUE, Sig_eps_prior_params, 
                                             N_mcmc = N_mcmc, adapt_frequency = 1000, adapt_min_scale = 0.1, accept_rate_target = 0.24, 
                                             proposal_scale_decay = 0.7, proposal_scale_init = 0.1)

idx_samples_keep <- floor(N_mcmc / 2)
theta_approx_samples <- post_approx_samples$theta[idx_samples_keep:N_mcmc,, drop = FALSE]
```

```{r}
plot(theta_approx_samples, type = "l", main = "Trace Plot", ylab = computer_model_data$pars_cal_names)
acf(theta_approx_samples, lag = 50, col = "red", main = paste0("Autocorrelation Plot: ", computer_model_data$pars_cal_name))
```


# Comparing Exact and Approximate Samples
```{r}
samples_list <- list(theta_samples, theta_approx_samples)

# Histograms
hist_plot <- get_hist_plot(samples_list, bins = 50, xlab = computer_model_data$pars_cal_name,
                           main_title = "Histogram of MCMC Samples")
print(hist_plot)

# plot(theta_approx_samples, type = "l", main = "Trace Plot", ylab = computer_model_data$pars_cal_names)
# acf(theta_approx_samples, lag = 50, col = "red", main = paste0("Autocorrelation Plot: ", computer_model_data$pars_cal_name))
# hist(theta_approx_samples, 100, main = "Histogram: MCMC Samples", xlab = computer_model_data$pars_cal_name)
```
...
N_mcmc <- 50000
post_samples <- mcmc_calibrate(computer_model_data$ref_pars, computer_model_data$pars_cal_sel, computer_model_data$data_obs, 
                               computer_model_data$output_vars, computer_model_data$PAR_data,
...
idx_samples_keep <- floor(N_mcmc / 2)
theta_samples <- post_samples$theta[idx_samples_keep:N_mcmc,, drop = FALSE]
plot(theta_samples, type = "l", main = "Trace Plot: KEXT", ylab = theta_param_name)
acf(theta_samples, lag = 50, col = "red", main = paste0("Autocorrelation Plot: ", theta_param_name))
hist(theta_samples, 100, main = "Histogram: MCMC Samples", xlab = theta_param_name)
```
...
emulator_tests <- run_emulator_comparison(computer_model_data, emulator_settings, theta_prior_params, train_test_data_settings, true_theta_samples = theta_samples)
theta_param_name <- emulator_tests$computer_model_data$pars_cal_names
```


```{r, echo = FALSE}
# Print posterior plots 
true_param_val <- emulator_tests$computer_model_data$ref_pars[emulator_tests$computer_model_data$pars_cal_sel, "true_value"]
