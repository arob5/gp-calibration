...
j <- 1
# Plot 1
plot_gp_fit_1d(theta_grid, SSR_test[,j], train_test_data$X_train, train_test_data$Y_train[,j], gp_pred[[j]]$mean, gp_pred[[j]]$sd2, 
               main = paste0("GP SSR Fit: ", test1_list$output_vars[[j]]), xlab = "KEXT", ylab = "SSR")
abline(v = test1_list$ref_pars[test1_list$pars_cal_sel, "true_value"], col = "gray", lty = "dashed")

# Plot 2: Truncate y axis
plot_gp_fit_1d(theta_grid, SSR_test[,j], train_test_data$X_train, train_test_data$Y_train[,j], gp_pred[[j]]$mean, gp_pred[[j]]$sd2, 
               main = paste0("GP SSR Fit: ", test1_list$output_vars[[j]]), xlab = "KEXT", ylab = "SSR", ylim = c(0, 1e6))
abline(v = test1_list$ref_pars[test1_list$pars_cal_sel, "true_value"], col = "gray", lty = "dashed")

# Plot 3: Log Scale
plot_gp_fit_1d(theta_grid, SSR_test[,j], train_test_data$X_train, train_test_data$Y_train[,j], gp_pred[[j]]$mean, gp_pred[[j]]$sd2, 
               main = paste0("GP SSR Fit: ", test1_list$output_vars[[j]]), xlab = "KEXT", ylab = "SSR", log_scale = TRUE)
abline(v = test1_list$ref_pars[test1_list$pars_cal_sel, "true_value"], col = "gray", lty = "dashed")
```
```{r, echo = FALSE}
j <- 1
...
# Plot 1
gp_plot <- plot_gp_fit_1d_ggplot(theta_grid, SSR_test[,j], train_test_data$X_train, train_test_data$Y_train[,j], gp_pred[[j]]$mean, gp_pred[[j]]$sd2, 
                                 main_title = paste0("GP SSR Fit: ", test1_list$output_vars[[j]]), xlab = "KEXT", ylab = "SSR", true_param = true_param_val, log_scale = TRUE)
gp_plot # + ylim(-1e5, 1e6)
```
...
plot(theta_grid, -lpred_pi_test, type = "l", main = "Posterior Loss", xlab = "theta", ylab = "Negative Predictive Density of Posterior Approx")
