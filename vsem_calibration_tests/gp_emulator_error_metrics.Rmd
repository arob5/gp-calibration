...
# Sampling method can be be "rectified" or "truncated". Anything else will be standard sampling. 
```{r, echo = FALSE}
# Emulator settings
emulator_settings <- data.frame(gp_lib = c("mlegp", "hetGP"), 
                                target = c("SSR", "SSR"), 
                                kernel = "Gaussian", 
                                sampling_method = c("rectified", "truncated"),
                                scale_X = TRUE, 
                                normalize_y = TRUE)
...
```{r}
# Computer model data
vsem_test_case_num <- 1
computer_model_data <- generate_vsem_test_case(vsem_test_case_num)
theta_param_name <- computer_model_data$pars_cal_names
```
...
emulator_tests <- run_emulator_comparison(computer_model_data, emulator_settings, theta_prior_params, train_test_data_settings, true_theta_samples = theta_samples, throw_neg_pred_err = FALSE)
