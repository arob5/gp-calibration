...
computer_model_data <- generate_vsem_test_case(5)
```
...
Sig_eps_prior_params <- list(dist = "IG", 
                             IG_shape = rep(IG_shape, 2), 
                             IG_scale = diag(computer_model_data$Sig_eps) * (IG_shape - 1))

prior_Sig_eps_samples <- matrix(NA, nrow = 10000, ncol = length(computer_model_data$output_vars))

for(i in seq_len(nrow(prior_Sig_eps_samples))) {
  prior_Sig_eps_samples[i,] <- diag(sample_prior_Sig_eps(Sig_eps_prior_params))
}
...
```{r}
N_mcmc <- 50000
samp_baseline <- mcmc_calibrate(computer_model_data, theta_prior_params, diag_cov = TRUE, learn_Sig_eps = TRUE, 
