...
# TODOs:
#    1.) write histogram function to automate the production of these plots. 
#    2.) look into why the trajectory alg posterior is multimodal, while the current PEcAn alg is not. 
#    3.) look into adding second Phi sampling step in the trajectory algorithm. 

# Approximate vs. True Samples. 
samp_approx_hist_gibbs <- as.matrix(select_mcmc_samp(mcmc_samp_dt, burn_in_start=burn_ins, 
                                                     test_labels="ind_gp_gibbs", param_types="theta")$sample, ncol=1)

samp_approx_hist_trajectory <- as.matrix(select_mcmc_samp(mcmc_samp_dt, burn_in_start=burn_ins, 
                                                          test_labels="ind_gp_trajectory", param_types="theta")$sample, ncol=1)


gibbs_hist_plot <- get_hist_plot(list(samp_exact, samp_approx_hist_gibbs), bins=50, xlab="u samples", ylab="density", 
                                      main_title = "True vs. Approximate Posterior Samples", data_names = c("exact", "gibbs"))
trajectory_hist_plot <- get_hist_plot(list(samp_exact, samp_approx_hist_trajectory), bins=50, xlab="u samples", ylab="density", 
                                           main_title = "True vs. Approximate Posterior Samples", 
                                      data_names = c("exact", "trajectory"))

plot(gibbs_hist_plot)
plot(trajectory_hist_plot)
