...
linear_Gaussian_info$true_posterior$SSR <- get_computer_model_SSR(computer_model_data, theta_vals=linear_Gaussian_info$true_posterior$mean, na.rm=TRUE)
...
# TODO: need to include points closer to the border of the parameter space in the design. Or at least use the points closer to the border 
#       to scale the input data, even if they are not included as design points. 
...
plot(prior_grid_info$inputs, lpost_marg_u, type="l", main="log p(u|Sigma, Y)") # , xlim = c(-1,1), ylim=c(-300,0))
plot(prior_grid_info$inputs, prior_grid_info$lpost, type="l", col="red", main="True lpost")

# Conclusion: posterior is being dominated by GP uncertainty here moreso than mean. And the posterior 
#             has lots of local minima due to GP uncertainty, making sampling difficult. 

```

```{r}
test_grid <- seq(1.571, 1.977, length.out=101)
lpost_vals <- dnorm(test_grid, linear_Gaussian_info$true_posterior$mean, sqrt(linear_Gaussian_info$true_posterior$Cov), log=FALSE)

plot(test_grid, lpost_vals, type="l")
plot(test_grid, log(lpost_vals), type="l")
...
init_design_info$init_estimates$best_input_idx <- bext_idx
...
samp_approx_hist <- as.matrix(select_mcmc_samp(mcmc_samp_dt, burn_in_start=burn_ins, test_labels="baseline", param_types="theta")$sample, ncol=1)

baseline_hist_plot <- get_hist_plot(list(samp_exact[1:N_mcmc_approx,,drop=FALSE], samp_approx_hist), bins=50, xlab="samples", ylab="density", 
                                         main_title = "Histogram", data_names = c("exact", "baseline")) 
...
mcmc_samp_dt <- format_mcmc_output(samp_list=mcmc_approx_gibbs[c("theta")], test_label="gibbs")
burn_ins <- c(gibbs=10000)
```
...
```{r}
# Approximate vs. True Samples. 
samp_approx_hist_gibbs <- as.matrix(select_mcmc_samp(mcmc_samp_dt, burn_in_start=burn_ins, test_labels="gibbs", param_types="theta")$sample, ncol=1)

gibbs_hist_plot <- get_hist_plot(list(samp_exact[1:nrow(samp_approx_hist_gibbs),,drop=FALSE], 
                                      samp_approx_hist_gibbs), bins=50, xlab="samples", ylab="density", 
                                      main_title = "Histogram", data_names = c("exact", "gibbs"))
