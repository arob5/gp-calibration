...
lnp_pred_log_scale <- predict_GP(train_test_data$X_test, lnp_fit, "hetGP", cov_mat = FALSE, denormalize_predictions = TRUE,
...
at the true value of KEXT (gray vertical line). However, the large scale on the y-axis masks the fact that the curve actually does 
achieve a minimum around 0.75, and around this minimum looks approximately quadratic. 
...
points(train_test_data$X_train, train_test_data$Y_train)
abline(v = ref_pars[pars_cal_sel, "best"], col = "gray")
```

```{r, echo = FALSE}
plot(train_test_data$X_test[test_order], train_test_data$Y_test[test_order], type = "l", col = "red", 
     main = "GP design and validation points", xlab = "KEXT", ylab = "SSR", ylim = c(0, 500))
points(train_test_data$X_train, train_test_data$Y_train)
abline(v = ref_pars[pars_cal_sel, "best"], col = "gray")
```
