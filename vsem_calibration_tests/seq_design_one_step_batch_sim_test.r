...
# Also need to pass in information to determine data used to compute validation metrics.
...
# other rounds use mean of approximate MCMC output. 
...
seq_design_one_step_batch_sim_test <- function(computer_model_data, prior_info, mcmc_info, emulator_settings, init_design_settings, 
  ...
  sig_eps_prior_params <- prior_info$sig_eps_prior_params
  
  # MCMC settings. 
  N_mcmc_exact <- mcmc_info$N_mcmc_exact
  N_mcmc_approx <- mcmc_info$N_mcmc_approx
