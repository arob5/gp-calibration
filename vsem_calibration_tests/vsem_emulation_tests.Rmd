...
train_test_data <- get_train_test_data(N_design_points, N_test, theta_prior_params, joint = TRUE, extrapolate = FALSE, 
                                       ref_pars, pars_cal_sel, data_obs, PAR, output_vars, scale_X = TRUE, normalize_Y = TRUE)
...
## Predict
```{r}
gp_pred_list <- predict_independent_GPs(train_test_data$X_test_preprocessed, gp_fit_list$fits, "hetGP", cov_mat = FALSE)
```

## Evaluate predictions
```{r}
# TODO: convert gp_pred errs back to original scale
calc_independent_gp_pred_errs(gp_pred_list, train_test_data$Y_test)
```
