...
# GP packages and kernel families to use
gp_libs <- c("mlegp", "hetGP")
gp_kernels <- c("Gaussian", "Gaussian")

# Number of design points used to fit GP emulators
...
X_design <- randomLHS(N_design_points, N_pars)
```

## Run Model and calculate L2 error
```{r}
model_outputs_list <- lapply(X_design, function(theta) run_VSEM(theta, ref_pars, pars_cal_sel, PAR, output_vars))
