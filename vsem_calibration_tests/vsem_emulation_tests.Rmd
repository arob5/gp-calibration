...
## Fit GPs
```{r}
gp_fit_list <- fit_GP(X_train_preprocessed, Y_train_preprocessed[,1,drop = FALSE], "hetGP", "Gaussian")
```

## Predict
```{r}
X_test_preprocessed <- scale_input_data(X_test, GP_data_preprocessed$input_bounds)
pred_list <- predict_GP(X_test_preprocessed, gp_fit_list$fit, "hetGP", cov_mat = FALSE)
```
